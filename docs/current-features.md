...
- `StompSsrmFields` — dedicated SSRM (pull-plane) editor: broker URL / listener topic / trigger destination + body / snapshot end token / **required Key Column** (Perspective table index; single column) / table name; inline validation via `validateStompSsrmConfig` (blank URL/topic/keyColumn, malformed ws URL, keyColumn missing from column definitions) rendered under each field (`role="alert"`, `data-testid="ssrm-*-error"`); deliberately exposes NONE of the push-plane knobs
