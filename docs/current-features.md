...
- **Grouping + aggregation (Phase 4):** `runQuery` branches to lazy server-side grouping when `rowGroupCols` is active — `level = groupKeys.length` returns distinct group rows for the next group column under the expanded path (with `valueCols` aggregated per group + `SSRM_CHILD_COUNT_FIELD` count), or the leaf rows once all group columns are consumed (sorted/sliced/shaped). Filters apply before grouping; AG-Grid `aggFunc` → engine aggregate (sum/avg/min/max/count). No protocol change (the grid already sends `rowGroupCols`/`groupKeys`/`valueCols`). `useSsrmDataSource().getChildCount` + `MarketsGrid` `serverSide` merge surface group child counts
