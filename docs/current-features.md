...
- **Provider SharedWorker (ADR Phase 4a–4d):** named `starui-provider:{appId}:{providerId}` (`createProviderWorker` / asset `provider-worker.mjs`); `ProviderHub` scopes monolith attach/delta/replay/idle-stop to one id; `createProviderClient` lazy singleton; `ProviderAppDataLookupCache` prefetches `{{name.key}}` from AppData SW on attach; `ensurePlatformReady({ providerWorkerScriptUrl })` / `DataHubProvider` / `useDataProvider` demux live subscribe off the monolith hub. star-demo passes `providerWorkerScriptUrl`. Monolith hub remains for catalog RPC, AppData mirror, and hub inspector
