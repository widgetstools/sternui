...
- `FormatterPicker` (compact toolbar mode) — type-aware trigger icon (`iconForDataType`) + human caption (raw Excel strings show as "Custom" with the format in the tooltip); popover body has a **type selector** (`onSelectType` re-categorizes a mis-inferred column without dropping to raw syntax), a **common/advanced tier split** (`FormatterPreset.tier` — trading-desk conventions like sign-coloring, parens-negatives, scientific, basis points and fixed-income ticks sit behind a "More formats" disclosure), **colored tile hints** (`FormatterPreset.hintSamples` renders the actual formatted output tinted via `toneColor` to match the grid's `[Green]`/`[Red]` Excel color map, instead of leaking raw color tokens), a **collapsible custom-Excel section** (auto-opens when the live value is a raw format with no matching preset; currency quick-insert row shown only for number/currency types), an **inline-expanding Excel reference** (`ExcelReferenceList` expands in-place instead of a popover-on-popover; the anchored `ExcelReferencePopover` is retained for the inline/editor presentation where there's no parent popover to stack on), and unified live-apply semantics (preset / currency / Apply all commit in place; dismiss via Escape / outside click; single Clear)
