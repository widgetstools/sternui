...
- `MAX UPDATES / SEC` (Performance band, `maxGridUpdatesPerSecond`, schemaVersion 6)
  ...
  `asyncTransactionWaitMillis` (default **8/sec** → 125 ms batching window;
...
- `useProviderDataWiring` — provider→grid hot path inside `MarketsGridContainer`; live ticks apply regardless of `document.hidden` — hidden/minimized blotters stay fully current (trading policy: window-local alerting + instant correctness on restore; the old hidden-pause + refresh-on-visible dormancy was removed, and star-demo's manifest disables Chromium background timer/renderer throttling to match); on STOMP auto-reconnect (`error` → `ready`) clears the stale banner and triggers `provider.refresh()` so every blotter replays the hub cache without a manual Reload
