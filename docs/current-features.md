...
- **Grid state** — serialise/restore AG Grid state; viewport-anchor restore
  RETRIES on `modelUpdated`/`firstDataRendered` until the row count covers the
  saved index (15 s deadline) — required under SSRM, where the count lands
  async (worklog T8)
- `MarketsGridProps.ssrmPullEngine` — externally-owned `SsrmEngine` for the
  pull data plane; forwarded through `MarketsGridHost`/`MarketsGridCore` to
  `SsrmMarketsGridSurface`, which passes `engine` and OMITS `rowData` (pull
  grids fetch viewport blocks only and must never `setRowData` into the
  shared table); `createPerspectiveEngine` + `SsrmEngine`/`PerspectiveClient`
  types re-exported from the package root for hosts

### 3.2 `@wellsfargo-starui/ssrm-grid`

**Path:** `packages/react-grid/ssrm-grid`
**Purpose:** Server-side-row-model grid — the grid pulls viewport-sized blocks
...
- `SsrmGrid.gridOptions` + `stripSsrmStructuralGridOptions` — module-
  ...
  `hostOverrideKeys` and no longer hard-nulls the host `statusBar`
- `translateSsrmStatusBar` + SSRM status panels (`ssrmStatusBarPanels.tsx`) —
  general-settings' client-side statusBar panel ids translate to SSRM
  stand-ins (`ServerTotalAndFilteredRowCountPanel`, `ServerFilteredRowCountPanel`,
...
- Quick-filter option intercept — `SsrmGrid` wraps `api.setGridOption` on
  ...
  `getGridOption` reads back for capture; a host-passed `quickFilterText`
  prop stays the controlled source (worklog T8)
...
- **`createPerspectiveEngine`** — Perspective-backed engine
  ([ADR](./ADR-ssrm-worker-hosted-engine.md)). The dataset lives once in a
  `Table` hosted by the provider SharedWorker; the window holds proxies only,
  so a blotter costs a viewport rather than a copy of the book (measured:
  **2.4 ms** for a 100-row block vs **5.85 s** for a 20k push). Maps AG Grid
  SSRM → Perspective: `sortModel`→`sort`, `filterModel`→`filter`/`expressions`
  (via `mapFilterModel`), `valueCols`→`aggregates`, `rowGroupCols`→`group_by`.
  `groupKeys` become **ancestor equality filters** plus `group_by` on the next
...
- `PerspectiveViewCache` / `viewCacheKey` — LRU cache of `View`s keyed by query
  ...
  that are not garbage collected**, so every eviction path calls `delete()`
  (covered by tests: LRU eviction, dispose, post-dispose insert, duplicate race)
- `PerspectiveClient` / `PerspectiveTable` / `PerspectiveView` — structural
  types for the injected client (engines are constructed against these, not
  against `@finos/perspective` classes)
- Default theme = the design-system `staruiGridTheme` (live OKLCH tokens,
  light + dark via `data-ag-theme-mode`); hosts that pass `props.theme`
  override it (worklog B5 — the old hardcoded dark-only quartz palette is
  gone)
...
- `SharedWorkerDataServicesHub` — worker state machine (providers, cache, fan-out); attach with matching **stable** `extra` overlay (e.g. same `asOfDate` / `rowShape`; `__refresh` ignored on the active slot for late-join) late-joins without a second upstream `restart`; re-sending the **same** catalog `cfg` on attach no longer forces RESTART+RECONFIG (only a changed cfg does); **`hydrateCatalog()`** preloads `ConfigCatalogCache` after ConfigManager init; **`get-config`** resolves the requested provider on demand (`ConfigCatalogCache.ensure`) so a grid attaches without waiting on the full preload; **`buildIntrospectSnapshot()`** / `hub-introspect` RPC for live provider + AppData diagnostics; fan-out is a **synchronous inline loop** over listeners (the dedicated fan-out worker pool was removed — with the window `MessagePort` pinned to the hub it added a serialize+deserialize round trip on the hub thread per listener and let async object deltas be overtaken by synchronous `delta-bin`; see [hub-fanout-optimizations.md §12](./hub-fanout-optimizations.md)); **`subscription-lost`** event + client auto re-attach when hub evicts stale subscribers; extended ping grace (`SUBSCRIBER_PING_TIMEOUT_HIDDEN_MS`) when client reports `meta.hidden`
...
- `ProviderClientAdapter` — client-side `IDataProvider`; cfg-free subscribe, `SnapshotReassembler` snapshot assembly, `getProvider()` on hub bundle. `getData()` returns the last snapshot commit by reference (not copied, not updated on live ticks). `onReset` deliveries update snapshot subscribers (mid-stream STOMP reconnect). `start()` resolves its one provider via the worker's on-demand `get-config` (single-row read, no full-catalog gate) so attach is race-safe even mid-preload. `restart()` is **make-before-break** (worklog B10): the replacement subscription attaches BEFORE the old one detaches, so the worker never sees a zero-subscriber gap — a same-overlay re-run late-joins the running provider (no upstream STOMP teardown/redial) and superseded handles go silent (their cancellation rejection is not forwarded as a provider error). Intentional upstream restart = `__reload` / `__forceRestart` only; `__refresh` late-joins by design
...
- **STOMP** (`startStomp()`)
  ...
  - Live conflation + trailing-edge throttle (`cfg.throttleMs` window; `cfg.conflateByKey` upsert key, defaults to `keyColumn`) via `bufferedDispatch()` — coalesces same-key ticks in the worker before fanout; **when `throttleMs` is omitted and `throttleEnabled` is not `false`, STOMP defaults to 50ms** (ADR Phase 0 hub safety net); `throttleMs: 0` = immediate passthrough; probe path bypasses it. Two explicit master switches (default ON): `cfg.throttleEnabled: false` fans out every delta immediately while keeping the `throttleMs` value; `cfg.conflateEnabled: false` disables conflation even when `keyColumn` could supply a key (the off-switch the `?? keyColumn` fallback otherwise prevented)
...
- **Attach/replay priority (ADR Phase 0):** while a late-join or refresh replay is in progress, post-ready live delta/`delta-bin`/`delta-patch` fan-out is deferred (last-write-wins per provider) and flushed after replay so ticks cannot interleave with replay chunks; production `installSharedWorkerHub` / `installProviderHub` also enables `deferLiveFanOut` so live fan-out is scheduled on a macrotask; **multi-chunk late-join replays yield between chunks** (`scheduleTask`) so a 20k-row peer attach cannot monopolize the provider SharedWorker (STOMP / pings / OpenFin tool-window opens stay responsive)
- **Peer blotter late-join (no STOMP tear-down):** attach with matching stable overlay (`rowShape` / `asOfDate`) — or a null recorded overlay + non-`__reload` peer — replays hub cache only; catalog cfg JSON noise on a ready/cached slot does not `RESTART+RECONFIG`; intentional Reload uses `__reload`
- **Hub-internal AppData + Config services (ADR Phase 1):** `AppDataService` (memory + optional Dexie) and `ConfigCatalogService` (wraps `ConfigCatalogCache`) are the only in-hub surfaces for template lookup, hydrate/resync/persist, and catalog get/list/invalidate; `createProvider` passes `appData.lookup`; `getCatalogService()` replaces raw cache exposure (`getConfigCatalog` deprecated alias). Wire protocol / `AppDataMirror` unchanged — prepares Phase 2–3 SharedWorker split
- **Config SharedWorker (ADR Phase 2):** named `starui-config:{appId}` (`createConfigWorker` / asset `config-catalog-worker.mjs`); `createConfigClient` lazy singleton; RPC ready/get/list/invalidate + `catalog-changed` fan-out; `wireConfigWorkerCatalogSync` keeps the Config SW cache aligned with main-thread ConfigManager writes; `ensureConfigReady({ configWorkerScriptUrl })` / `ensurePlatformReady({ configWorkerScriptUrl })` connect it for P1 tool windows without the data hub; **every RPC (including the `config-ready` handshake on the first-paint path) times out after `CONFIG_CLIENT_TIMEOUT_MS` (30s)** — a missing/CSP-blocked worker asset fires `error` on the SharedWorker rather than throwing, so without it bootstrap hangs on a blank shell; **`config-ping` heartbeat + `sweepStalePorts()`** evict ports whose window closed (`MessagePort` has no close event and posting to a dead port doesn't throw, so the port set otherwise grows for the life of the origin). Data hub catalog cache remains for streaming attach (dual invalidate until Phase 2 single-writer follow-up)
- **AppData SharedWorker (ADR Phase 3):** named `starui-appdata:{appId}` (`createAppDataWorker` / asset `appdata-worker.mjs`); `createAppDataClient` lazy singleton; reuses `appdata-*` wire + `AppDataMirror` attach; adds `appdata-lookup` RPC for cross-process template resolution; `ensureConfigReady({ appDataWorkerScriptUrl })` / `ensurePlatformReady({ appDataWorkerScriptUrl })` warm it for P1/P2 profiles; **RPC timeout `APPDATA_CLIENT_TIMEOUT_MS` (30s)** on the `appdata-worker-ready` handshake so a dead worker asset falls back instead of hanging bootstrap; **`appdata-ping` heartbeat + `sweepStalePorts()`** evict closed windows' ports *and their mirror subscriptions* (otherwise every write fans deltas out to dead windows forever); **`hubAppDataDisabled` is derived from the connected client, not from `appDataWorkerScriptUrl`** — the monolith is spawned only after the connect resolves in demuxed apps, so a failed AppData SW falls back to in-process AppData instead of leaving no authority (empty snapshots, `{{name.key}}` → undefined, nacked writes). Data hub still keeps in-process `AppDataService` for streaming providers (dual until Phase 4)
- **Provider SharedWorker (ADR Phase 4a–4d + control-plane + single-writer):** named `starui-provider:{appId}:{providerId}`; demux via `providerWorkerScriptUrl`; `hubStreamingDisabled` / `hubAppDataDisabled` on monolith when provider/AppData workers are wired; Config SW is sole catalog invalidate + React catalog reads when present; hub introspect reports `topologyNote` for demuxed apps; `installProviderHub` uses the same inline fan-out loop as the monolith. Monolith retained for legacy non-demuxed callers and thin control-plane catalog hydrate
- **`ProviderTableBridge` (pull data path, [ADR](./ADR-ssrm-worker-hosted-engine.md)):** writes a provider's row stream into a worker-hosted Perspective table **once** instead of fanning rows to every window — `snapshot()` → `replace`, `push()` → keyed `update` (the table's `index` does insert-vs-update, as `keyColumn` does for the hub cache). Live frames are **conflated by key and flushed on a timer** (default 100ms, matching provider `throttleMs`) so cost tracks flush rate rather than tick rate — a 20k row/s sweep becomes one write per window; partial/thin frames **merge** rather than overwrite; a `snapshot()` **drops coalesced rows** so a replaced book cannot resurrect them; writes are **serialised** so a later frame cannot overtake an earlier one; write failures are reported and the bridge keeps running. Typed against a local `BridgeTable` structural interface — `host-data` must not import from `react-grid` (see ARCHITECTURE import rules). Does **not** delete the table on dispose: it is shared with every attached window
- **`perspectiveSharedWorkerName` / `linkProviderToPerspective` / `parsePerspectiveWorkerName` / `resolvePerspectiveWorkerUrl`:** engine worker is `starui-psp:{appId}:{providerId}` (Perspective's prebuilt server worker). Chromium does not expose `SharedWorker` inside worker scopes, so a **window performs the introduction**: it opens the engine worker and **transfers** that `MessagePort` to the provider worker (`psp-attach`), after which rows flow provider→engine and the main thread is out of the data path. Each window opens its **own** connection to the same named worker for reads, so one server and one table serve every blotter; duplicate attaches from racing windows are ignored
- **`PerspectiveAttachHandler`:** provider-worker side of the link — takes the transferred port, stands up a Perspective client, opens (or creates with `index: keyColumn`) the provider's table, and exposes a `ProviderTableBridge` per provider. **Exactly one link survives** when several windows race the hand-off (each window must try, since none can know if another got there first); late attaches are acked `linked: false` and their ports closed, so N windows never become N writers against one table. Failed attaches ack with the error, close the port, and leave the provider retryable. **Table schema** comes from the attach request (`PerspectiveAttachRequest.schema`) or an async `schemaFor` fallback; the key column is injected if missing (Perspective aborts on an index absent from the schema), and a schema-less create acks a retryable error. `onLinked` fires before the ack so the host can seed the fresh table. WASM/handshake specifics are injected as `connect`, so dedupe / table open-vs-create / bridge lifecycle are unit-testable without a browser
- **Pull-path wiring in `installProviderHub` (worklog T3):** `psp-attach` messages route to a `PerspectiveAttachHandler` (built when `perspectiveConnect` is passed; `providerWorkerEntry` always passes `createProviderPerspectiveConnect`) ahead of the hub protocol; on link the table is **seeded from the hub row cache** (`getCachedRows`) and every subsequent provider frame **tees into the bridge** via the hub's `pullSinkFor` hook (`replace` → `bridge.snapshot`, live → `bridge.push`, thin deltas included — partial rows are keyed merges). The cache stays authoritative for push subscribers; the tee adds the pull path without removing anything. Schema for new tables is **inferred from the first cached rows** (`inferPerspectiveSchema`, waiting up to 30s for the snapshot when the link races it). Verified live against `dev:stomp`: 20k-row table seeded in the engine worker, 25-row block reads ≈ **1.1 ms** from a window holding no dataset, live updates flowing worker→worker with no window in the data path
- **`createProviderPerspectiveConnect`:** the production `connect` — fetches `perspective-js.wasm` / `perspective-server.wasm` as **siblings of the provider worker bundle**, unwraps Perspective's `pro_self_extracting_wasm` **stage-0** packaging (extractor inlined; both artifacts ship stage-0 wrapped), and initialises the **standalone** `dist/wasm/perspective-js.js` module — the `@finos/perspective` root inlines a private copy of that glue, so initialising the root while constructing `Client` from the standalone module leaves the WASM state split-brained. Assets fetch lazily on the first hand-off and are cached as ArrayBuffers
- **`connectPerspectivePort` / `PERSPECTIVE_HANDSHAKE_TIMEOUT_MS` (30s):** the real `connect` — drives Perspective's `{cmd:'init', id, args:[serverWasm]}` handshake on the transferred port, awaits the `{id}` ack, then bridges `Client.send_request` → `postMessage` and inbound frames → `handle_response`. Request bytes are **sliced out of the WASM heap** before transfer (the view is not stable). **Times out rather than hanging** — a missing/CSP-blocked worker asset fires `error` on the SharedWorker instead of throwing — and detaches its listener on timeout. Mirrors `@finos/perspective`'s own `worker()` wiring, which can't be reused directly because it constructs a `SharedWorker` (not available in worker scopes)
- **Engine worker asset:** `perspective-server.worker.mjs` is Perspective's prebuilt server worker, copied into `dist/assets/` by `scripts/buildWorker.mjs` so it sits beside `provider-worker.mjs` for `resolvePerspectiveWorkerUrl` (also a package export: `@wellsfargo-starui/host-data/assets/perspective-server.worker.mjs`). `perspective-js.wasm` + `perspective-server.wasm` are copied alongside for `createProviderPerspectiveConnect`. Copied rather than bundled — bundling would require Perspective's internal `wasm/engine` modules; the published artifact is the supported entry point. Build warns and skips if `@finos/perspective` is unresolvable
- **`createPerspectiveReadClient({ appId, providerId, workerScriptUrl })`** — window-side boot for the pull plane (ADR Phase 4): once-per-window WASM init (`init_server`/`init_client` from siblings of the engine worker asset) + `perspective.worker()` over the shared engine worker; returns a structurally-typed client for `createPerspectiveEngine` (host-data must not import from react-grid). `@finos/perspective` is imported LAZILY — its Node entry compiles WASM at module eval, which would break any vitest/jsdom test that merely imports this dependency chain
- **`perspectiveWorkerScriptUrl`** threaded through the bootstrap: `EnsurePlatformReadyOpts` → `EnsureHubOpts` → `ProviderWorkerRoutingOpts` → `CreateProviderClientOpts`; when present alongside `providerWorkerScriptUrl`, pull-capable consumers (`MarketsGridContainer dataPlane='pull'`) can attach worker-hosted tables
