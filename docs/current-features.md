...
- **Provider SharedWorker (ADR Phase 4a):** named `starui-provider:{appId}:{providerId}` (`createProviderWorker` / asset `provider-worker.mjs`); `ProviderHub` scopes monolith attach/delta/replay/idle-stop to one id; `createProviderClient` lazy singleton + `subscribe` via `SharedWorkerDataServicesClient`. Demos still use the monolith hub; Config/AppData bridges and façade cutover are follow-ups
