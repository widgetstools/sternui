...
#### SSRM pull plane (`@starui/ssrm-grid/pull`) — V2, P2–P4b-2
...
- `connectSsrmProvider({ appId, providerId, workerUrl, config, wasm? })` — control port (`SsrmControlClient`; configure idempotent worker-side) + direct Perspective data client (`perspective.worker(sharedWorker)`, lazily imported); exposes `state`/`onState` (replay + broadcasts), `configure`, `restart`, `updateRows(rows, generation?)` (P4b cell-edit write-back; generation defaults to the latest known), `openTable()` (memoized, retries while the seed creates the table), `dispose`; test seams `createWorker`/`openDataClient`
- `createSsrmPullDatasource({ connection, keyColumn, columns?, quickFilterColumns? })` — AG `IServerSideDatasource`: rowCount semantics from DatasetState never inferred (`seeding` delivers a flat root store without finalizing + reopens the lazy count; grouped/child stores always finalize per-load with the view count — AG #28 forbids `setRowCount` under grouping; `live`/`empty` finalize, `error` fails); generation fencing drops stale responses; viewport block LRU (default 32 — sized above the grid's `maxBlocksInCache` so AG evictions never bypass serve-then-refresh) + serve-then-refresh (cache hit answers synchronously, fresh read lands via index-addressed route-aware `applyServerSideRowData`); ticks refetch the MRU `maxSweepBlocks` cached blocks (narrow 6 / wide 4 — LRU recency order ≈ viewport order) of the current generation (flat, group-level, expanded-leaf) and patch via `applyServerSideTransactionAsync({ route, update })` keyed by `keyColumn` (leaves) / group path id (group rows) — throttled bare `on_update` on a root-route view, never purges; seeding→live refreshes loaded blocks without purge
- Pull datasource interactive-perf hardening (2026-07) — **plan-epoch fence**: the epoch bumps on every effective query-shape change (quick-filter apply; root filter/sort fingerprint change on a root load) and every async block read (cold miss, serve-then-refresh, tick sweep, prefetch) captures + re-checks it, dropping stale-shape results before any cache write / `applyServerSideRowData` / transaction / `setRowCount` (kills transient wrong-rows on filter churn; a root load whose shape went stale mid-read re-serves the current shape so AG's callback is always answered); **scroll-aware sweep deferral**: `datasource.onScroll()` (consumers wire AG `onBodyScroll`) defers tick sweeps during scrolling + a `scrollSettleMs` window (default 300 ms) and while any cold miss is in flight, coalescing to ONE sweep on settle; **neighbor prefetch** (design fact #6): a resolved cold miss on a flat leaf plan warms adjacent block(s) into the BlockCache (cache-only) so directional scrolling stays a cache hit; **quick-filter view pre-warm**: the settled apply builds the new shape's Perspective view concurrently with AG's refresh cycle; **shrink-only `redrawRows`** judged against the last datasource-delivered total (never `getDisplayedRowCount`, which counts footer/loading rows); `getStats()` exposes monotonic read/paint counters (sweep runs/reads, miss/refresh/prefetch reads, stale drops, redraws, deferrals) for perf probes and tests
- Row grouping (P4a) — group-level requests served from a Perspective `group_by` view on the request's NEXT level (ancestor equality filters pin the route; multi-level verified live); group rows carry the group label, `valueCols` aggregates (sum/min/max/avg/count/first/last/distinctCount + weighted mean — see the weighted-aggregation bullet; unknown aggFuncs reported), leaf child count (`CHILD_COUNT_FIELD` → AG `getChildCount`; key column's `count` aggregate) and a refresh-stable path id (`GROUP_ID_FIELD`, `encodeGroupRowId`); consumers wire `getRowId` via `createSsrmRowIdGetter(keyColumn)`; reads skip Perspective's leading `__ROW_PATH__: []` total row (`num_rows − 1` = group count); `'ag-Grid-AutoColumn'` sorts map to the level's group field (label sort via `unique` aggregate), leaf plans drop it
- Grand total (P4a) — AG 36's native `grandTotalRow` fed via `LoadSuccessParams.grandTotalData` on root loads answering `needsGrandTotal` (chosen over a hand-rolled pinned row: the grid owns `GRAND_TOTAL_ROW_ID` and its placement), read from a single-group rollup view (`group_by` on a constant expression) over the SAME filtered set; ticks keep it live via `rowNode.updateData` + `refreshCells` on the grand-total node (it lives outside every store — transactions can't reach it); consumers need `RowApiModule` + `RenderApiModule`
- `buildQueryPlan` / `buildRollupPlan` — AG request → Perspective view config (`sort`/`filter`/`group_by`/`aggregates`/`expressions`); plans carry `kind`, `route` (AG store route) and `group` info; `canonicalViewKey` cache identity covers expressions + aggregates
- `agFilterModelToPerspective` (P4a full simple-filter parity) — native ops where Perspective has them (text equals/notEqual/contains/startsWith/endsWith — engine-verified case-INSENSITIVE literal matches, AG's default text semantics; number comparisons + inRange; date equals/before/after/inRange/blank/notBlank with date-only normalized terms, `dateString` via lexicographic ISO compare; set → `in`; AND-combined flattened); boolean EXPRESSION columns filtered `== true` for what clause-lists can't say: per-column OR-combined conditions (Perspective `filter_op` is view-global so mixed AND/OR is natively inexpressible — the per-column expression keeps OR-within-column AND-across-columns exactly), `notContains` (null-safe `not(match(lower(…)))`), set selections including null (`string()`-cast equality); sub-day date terms/unknown ops/advanced models reported `unsupported`, never guessed
- `filterExpressions` — expression rendering: `regexNeedle` (regex-escape + ExprTK-escape + lowercase), `conditionExpr`, `setWithNullExpr`, `quickFilterExpr` (every token must match ≥1 configured column), reserved `__ssrm_*` aliases
- Quick filter (P4a) — `datasource.setQuickFilter(text)`: case-insensitive multi-token match across `quickFilterColumns` folded into every plan as an expression column; refreshes loaded blocks without purging; empty/none configured → loud `unsupported` report
- Set-filter values (P4a) — `datasource.getDistinctValues(field)`: transient `group_by` view, group labels (may include null), whole-table (AG set-filter convention), view deleted after read
- Cell-edit write-back (P4b, `editRows.ts`) — `createSsrmCellEditHandler({ connection, keyColumn })` for AG `onCellValueChanged`: posts ONE keyed partial row via `connection.updateRows` stamped with the generation the edit was computed against (worker fences stale generations and schema-coerces values — string editor input on a float column comes back a number on the next tick-refresh cycle, in EVERY window); refuses key-column edits (re-keyed write = insert, not update) and rows without the key (group rows); worker refusals warn, never throw. Bulk shapes: `fetchLoadedRowsOrRefuse(reader, keys)` — the fetch-or-refuse guard (all target rows loaded, or ONE `console.warn` + `null`, never a silent subset edit) and `updateLoadedRowsOrRefuse({ connection, reader, keyColumn, edits })` built on it (merges per-key edits, posts one batch)
- `datasource.queryAll({ chunkSize?, columns?, onChunk? })` (P4b) — the ENTIRE current filtered+sorted set (grouping stripped to leaf rows; filter model + sort model + quick filter kept) read in bounded windowed chunks (default 10k rows/read) over a TRANSIENT Perspective view (never the LRU); `onChunk` streams with bounded memory; generation change mid-read rejects; the data plane for full-set export + charting (AG's own SSRM export/integrated charts walk only loaded blocks)
- Full-set export (P4b, `exportRows.ts`) — `rowsToCsv(rows, columns, { includeHeader?, delimiter? })` + `csvEscapeCell` (RFC 4180): the library route for CSV is DIRECT sheet building over `queryAll` (streamable, no second grid); the Excel route is AG's `ExcelExportModule` on a transient off-screen client-side grid fed with `queryAll` rows (consumer recipe — see the lab spike's `exportFilteredExcel`); route rationale documented in the module header
- Tree data (P4b-2) — datasource opt `treePathFields` (the config's field) synthesizes a serverSide tree from ordered categorical fields (level i groups by `treePathFields[i]`; a full-depth route reads leaf rows) — no natural parent/child column needed; AG 36 tree requests carry NO `rowGroupCols` (only `groupKeys` from `getServerSideGroupKey` up the expanded path), so tree levels are served by the SAME group-level plans as row grouping (ancestor filters pin the route, stable path-encoded row ids); the grid wires the AG contract via `isSsrmServerSideGroup` (rows with a stamped path id expand) and `getSsrmServerSideGroupKey` (the row's own `GROUP_KEY_FIELD` stamp); `queryAll` strips tree levels to leaves exactly like grouping; misconfigured `rowGroupCols` + `treePathFields` together → row grouping wins. The other tree shape is `treeParentField` (natural parent-id adjacency, below) — the two are mutually exclusive
- Master-detail (P4b-2, `detailRows.ts`) — `createSsrmDetailFetcher({ connection, keyColumn, detailQuery? })` answers AG 36's serverSide masterDetail contract (`detailCellRendererParams.getDetailRowData`; successCallback-only params, so failures warn + deliver an EMPTY detail set — the panel never hangs): default detail source is a keyed single-row read of the hosted table over a transient view (the detail always shows the table's CURRENT values, not the block's copy); richer sources plug in via the `detailQuery(masterRow, { table, keyColumn, key })` hook; `createSsrmRowMasterGetter(keyColumn)` for AG `isRowMaster` (group/keyless rows never expand)
- Calc/expression columns (P4b-2) — datasource opt `calcExpressions` (the config's field; name → Perspective ExprTK expression over REAL columns) flows into every view the plan builder emits, so calc columns sort / filter / aggregate (grouped views + grand-total rollup) / export (`queryAll`) / distinct (`getDistinctValues`) like real columns (engine-verified: column-omitted views serve expression columns as payload; native filter ops, sorts, `group_by` and aggregates all accept the alias); reserved `__ssrm_` names are refused + reported; expressions canNOT reference other expression aliases (engine limit) so OR-combined/`notContains`/set-with-null filters on calc columns are dropped + reported, never guessed; plans with EXPLICIT `columns` (group-level, rollup, configured-column rows) prune unreferenced calc expressions — Perspective computes every attached expression per row (measured ~120 ms off a 20k group load)
- Wide-book delta gate (P4b-2, `sweepGate.ts`, design fact #5) — `resolveSweepGate(columnCount, { tickRefreshMs, wideColumnThreshold, sweepThrottleWideMs })`: at/above `wideColumnThreshold` columns (default 80; configured width or observed from the first leaf read) the tick sweep degrades to `sweepThrottleWideMs` (default 1000 ms, clamped ≥ the narrow throttle) and refetches only the `WIDE_SWEEP_MAX_BLOCKS` (4) most-recently-used cached blocks (block-LRU recency IS viewport order — off-screen blocks catch up via plain cache-miss reads on scroll-back); narrow books keep the full refetch-everything sweep; pure decision + datasource-level MRU-sweep both unit-tested
- Weighted aggregation (2026-07) — datasource opt `weightedAggregates` (the config's field; value field → WEIGHT field) serves AG's `wavg` / `weightedAvg` / `weightedMean` value columns as Perspective's `['weighted mean', [weightField]]` tuple (there is no bare `wavg` in this engine build — it aborts with "unknown aggregate operation"); AG's `valueCols` carries no weight slot, so the pairing can only come from config. A weighted column with NO weight entry is reported `unsupported` and dropped — never downgraded to a plain average, because a weighted spread served unweighted is a wrong number that looks right. Same refusal at the catalog seam (`validateStompSsrmConfig`)
- Extended aggregate coverage (2026-07) — `AGG_FUNC_MAP` adds `first` / `last` (AG built-ins, same names in Perspective) and `distinctCount` → `'distinct count'`, alongside sum/min/max/avg/count; unknown aggFuncs still reported, never guessed
- Parent-id tree data (2026-07) — datasource opt `treeParentField` (the config's field): a serverSide tree from a NATURAL adjacency column, for data whose hierarchy already exists in the book. Every level is a filtered LEAF read (root = `[parentField, 'is null', null]`; an expanded route = rows whose parent is the route's last key), so rows keep their natural key as the AG row id instead of a synthesized path id. The route clause is PREPENDED and `filter_op` deleted, so neither the quick-filter OR fast path nor a user filter can widen a level to the whole book. Which rows expand is answered from ONE `group_by` histogram over the parent column — that single read yields both the has-children set and each parent's child count — cached per generation and refreshed no more often than `treeChildCountRefreshMs` (default 2000 ms), because live inserts create new parents but a per-row question would be ruinous. Mutually exclusive with `treePathFields` (the synthesized shape); the catalog seam refuses both together rather than picking one silently
- Column-projection narrowing (2026-07) — datasource opts `projectDisplayedColumns` + `alwaysProjectColumns` (both config fields): reads only the columns the grid is DISPLAYING rather than every table column. Engine-verified safe — filter, sort, `group_by` and expressions all operate on columns that are not projected, so only what is rendered has to travel — and it lets `buildQueryPlan` prune calc expressions the view never references. Measured view build 359 ms @ 34 columns vs 750 ms @ 158 on 100k rows. OPT-IN, because a cell renderer reading a SIBLING field that is not itself displayed would find it missing: those go in `alwaysProjectColumns`. Falls back to the full projection whenever the grid has not reported displayed columns yet, so an early read is never starved
- Quick-filter native `contains` fast path (2026-07) — a SINGLE-token quick filter that is the ONLY filter rides Perspective's native `contains` with a view-global `filter_op: 'or'`, skipping the ExprTK expression column entirely (literal case-insensitive match, no regex, no per-cell allocation): measured 191 ms vs 235 ms at 100k × 34. Strictly gated — `filter_op` is VIEW-GLOBAL, so with any other clause present (a column filter, or an ancestor route filter under grouping) an OR would silently WIDEN instead of narrowing; multi-token also falls back, since tokens AND together and one global op cannot express AND-of-ORs
- Sorted-block reflow (2026-07) — when a tick RE-ORDERS a sorted block, a keyed update transaction cannot express it (the surviving keys are the same; only their positions moved) and `diffRowsByKey` cannot detect it either, so the block is replaced index-addressed via `applyServerSideRowData` rather than transacted. Left as a transaction the reorder is invisible and the grid paints a correctly-valued, wrongly-ordered block
- Live aggregates at EVERY level (2026-07) — the tick sweep refreshes group-level and rollup shapes, not just the root: shapes the user is LOOKING at are re-ACQUIRED through `ViewCache.withView`, never merely peeked, because peeking does not refresh LRU recency, so an intermediate group level or the grand total drifts to the tail, gets evicted, and silently freezes while the leaf rows keep ticking (Constraint C5). Covered by a DOM-asserting e2e that watches every row-group level tick
- SSRM status-bar stand-ins (2026-07, package root export) — `SSRM_DEFAULT_STATUS_BAR` + `ServerTotalAndFilteredRowCountPanel` / `ServerFilteredRowCountPanel` / `ServerSelectedRowCountPanel`: AG 36's built-in row-count status panels are clientSide-ONLY and are silently *filtered out* under `rowModelType="serverSide"`, leaving an empty status bar (Constraint C4). The stand-ins reproduce AG's exact DOM/classes and read their counts from grid `context` (`{ totalRowCount, filteredRowCount }`), which the datasource owner keeps current
- `ViewCache` — LRU (8) of live views keyed by query shape; `peek()` for tick refreshes (never resurrects evicted shapes); evicted views `delete()`d; memoized creation
- `BlockCache` — viewport block LRU (12), generation-stamped (design fact #6 anti-jank); `recentEntries(generation, limit)` — the MRU slice across every view shape (the wide-gate sweep set)
- `PullTable`/`PullView`/`PullViewConfig`/`PullDatasourceConnection` — structural vendor surfaces so the datasource unit-tests with injected fakes (shared grouped-view fake in `tests/pull/fakePerspective.ts`)
- Re-exports `toSsrmDatasetConfig` + `StompSsrmProviderConfig` (from `@starui/host-data/runtime/ssrm`) so pull consumers wire a catalog row to the worker from one import
- Lab spike `apps/demos/markets-grid-lab/spikes/ssrmGrid.html` — real AG enterprise grid mounted once per `(providerId, generation)`, gated on DatasetState; headless-verified: progressive seed fill → live 20k, sort ≤250 ms, tick repaint with zero loading stubs, second-tab attach with zero re-stream, solo-tab reload repopulating from the live table, restart → remount + refill. P3: the spike seeds a `stomp-ssrm` catalog row through `DataProviderConfigStore` (upserted each load so URL knobs apply), reads it back, validates (`validateStompSsrmConfig`) and maps it (`toSsrmDatasetConfig`); the row's `columnDefinitions` drive both the table schema and the grid columns; the catalog row id is the SharedWorker `providerId`. P4a: grouping probes (`groupBy`/`expandDisplayedRow`/`displayedRow`/`grandTotalData`/`setQuickFilter`/`getDistinctValues`/`countRows` control reads), `grandTotalRow: 'bottom'`, set filter on `bookName` fed by `getDistinctValues`, quick-filter box (design-system `Input`); headless-verified on the 20k live book: group 241 ms / expand 146 ms (budgets 600/250), all 5 group aggregates + grand total ticking over 5 s with 0 loading stubs and no remount, OR-filter and OR+inRange row counts exactly matching direct-table control reads, quick filter narrowing 20000→4002 and clearing back. P4b: editable non-key columns wired to `createSsrmCellEditHandler`, full-set action buttons (design-system `Button`: CSV download via `rowsToCsv`, Excel via off-screen grid + `ExcelExportModule`, PnL chart via AG Charts standalone `AgCharts.create` over `queryAll` — integrated charts under SSRM only see loaded blocks) + probes (`setCellValue`/`queryAllCount`/`exportFilteredCsv`/`exportFilteredExcel`/`chartFilteredSet`); headless-verified over two tabs on the 20k live book: trader edit A→B in 339 ms, string `'777.25'` typed into the float `quantity` column lands as number `777.25` in BOTH tabs (schema coercion round-trip), and under an active 2-book set filter CSV rows = Excel off-screen grid rows = chart points = 7995 = the direct-table control count (loaded-block ceiling 1000). P4b-2: URL knobs `?tree=1` (treeData grid wired to `isSsrmServerSideGroup`/`getSsrmServerSideGroupKey` over the catalog row's `treePathFields: ['bookName','trader']`) and `?master=1` (masterDetail via `createSsrmDetailFetcher` + `createSsrmRowMasterGetter`, `agGroupCellRenderer` on the first column), the catalog row's `calcExpressions: { pnlPerUnit: '"pnl" / "quantity"' }` as a first-class grid column, probes (`queryAllRows`/`detailGrids`/`readRowByKey`); headless-verified on the 20k live book: tree root 5 book nodes with child counts exactly matching direct-table control reads (Σ 20000), trader level 6/6 exact, leaves pinned to their two-key route; expanded master row's detail grid = the table's current row (identity + numerics exact vs control); calc column sorts (ppu == pnl/qty on every sampled row), grouped sums + grand total BIT-EXACT vs client-side Σ pnl/qty in a tick-quiet window, and rides queryAll/CSV for all 20000 rows; timing re-probe group 205–267 ms (P4a 241) / expand 43–113 ms (P4a 146)
...
- `ConnectionTab` — connection string, auth, transport selection; "Test Connection" button (STOMP/STOMP SSRM/REST) drives `useProviderProbe.test()`. STOMP (and `stomp-ssrm`, whose transport subset is field-identical) runs a pure socket connect (`connectStomp` — handshake only, no subscribe/trigger/rows) and shows "Connected"; row-fetching transports (REST/mock) show "Connected — received N rows". `stomp-ssrm` also rides `probeStomp` for Infer Fields
...
- `StompSsrmFields` — dedicated SSRM (pull-plane) editor: broker URL / listener topic / trigger destination + body / snapshot end token / **required Key Column** (Perspective table index; single column) / table name, plus every window-side query knob the pull datasource takes: **Calculated Columns** (name → Perspective-expression rows via the shared `KeyValueEditor`; clears to `undefined` when the last row is removed), **Weighted Aggregates** (value column → weight column rows, same editor), **Tree Data** (ordered level rows — add/edit/remove, index-numbered via the local `ColumnListEditor` — plus the **Tree Parent Column** input for the natural parent-id shape; a config setting both shows the conflict under BOTH), **Column Projection** (a `Switch` for `projectDisplayedColumns` — dropped to `undefined` rather than stored `false` when off — over an unnumbered `ColumnListEditor` of always-projected columns) and the **Wide-Book Refresh Gate** (`wideColumnThreshold` / `sweepThrottleWideMs` numeric inputs, empty = defaults); inline validation via `validateStompSsrmConfig` rendered under each field/card (`role="alert"`, `data-testid="ssrm-*-error"`); deliberately exposes NONE of the push-plane knobs (fan-out/conflation/wire-format)
- `MockFields` — seed data, latency, mutation playback
- `AppDataFields` — read from `@starui/host-data` AppData
- `BehaviourFields` — per-transport behaviour knobs; STOMP: reconnect initial delay, realtime throttle (on/off switch + ms) + conflation (on/off switch + conflate-by-key), "Thin field-level deltas" switch (`thinDeltas`), snapshot chunk size, "Wire format" select (`wireFormat`: JSON / Columnar), "Keep only column fields" projection switch (`projectFields`) (all written to `cfg`, also settable in code); STOMP SSRM: only the knobs the provider worker consumes — max buffered pre-table rows (`maxBufferedRows`) + heartbeat outgoing/incoming, with an explicit note that the pull plane has no fan-out/throttle/conflation settings
...
#### Data-provider contracts (`@starui/shared-types/dataProvider`, re-exported by `@starui/types`)

- `PROVIDER_TYPES` / `ProviderType` — `stomp`, `stomp-ssrm`, `rest`, `websocket`, `socketio`, `mock`, `appdata`; `PROVIDER_TYPE_TO_COMPONENT_SUBTYPE` / `COMPONENT_SUBTYPE_TO_PROVIDER_TYPE` maps; `DEFAULT_PROVIDER_CONFIGS` + `getDefaultProviderConfig()`
- Per-transport config interfaces (`StompProviderConfig`, `RestProviderConfig`, `WebSocketProviderConfig`, `SocketIOProviderConfig`, `MockProviderConfig`, `AppDataProviderConfig`) unioned as `ProviderConfig`; `DataProviderConfig` catalog wrapper; `validateProviderConfig()`
- `StompSsrmProviderConfig` (`providerType: 'stomp-ssrm'`, module `stompSsrm.ts`) — the SSRM pull-plane catalog config: transport (`websocketUrl`/`listenerTopic`/`requestMessage`/`requestBody`/`requestHeaders`/`snapshotEndToken`), **required single-column `keyColumn`** (Perspective table index), `columnDefinitions` (single declaration of table schema + grid columns), `inferredFields`, `tableName`, `heartbeat`, `maxBufferedRows`; window-side knobs (consumed by `createSsrmPullDatasource`/the grid, never the worker): `calcExpressions` (name → Perspective expression), `treePathFields` (synthesized serverSide tree) and `treeParentField` (natural parent-id tree — mutually exclusive with `treePathFields`), `weightedAggregates` (value field → weight field, for `wavg` value columns), `projectDisplayedColumns` + `alwaysProjectColumns` (read only rendered columns, plus the sibling fields cell renderers need), `wideColumnThreshold` + `sweepThrottleWideMs` (wide-book delta gate); `reconnect` reserved (documented: the V2 worker never silently redials — restart is explicit). Carries NONE of the push-plane fan-out/throttle/conflation/wire knobs
- `validateStompSsrmConfig(cfg)` — pure structured validation (`StompSsrmValidationIssue[]`: field + code + message): missing/blank `websocketUrl`/`listenerTopic`/`keyColumn`, malformed non-`ws(s)://` URL (skipped for template-token values), composite-array `keyColumn`, `keyColumn` absent from declared `columnDefinitions`; window-side knobs: calc-expression names blank / colliding with declared columns / under the reserved `__ssrm` prefix / cross-referencing another calc alias (engine limit) and blank expressions; tree levels blank / duplicated / absent from declared columns; `treeParentField` blank / absent from declared columns / set alongside `treePathFields` (`tree-mode-conflict`, reported under BOTH fields — two incompatible hierarchies are refused, never resolved by a silent precedence); `weightedAggregates` blank value column, a value column with NO weight (`weight-missing` — the catalog-seam twin of the datasource's refusal to downgrade a weighted mean to a plain average), a weight absent from declared columns (`weight-not-in-columns`) or whose declared `cellDataType` is not `number` (`weight-not-numeric`); `alwaysProjectColumns` entries blank / absent from declared columns; `wideColumnThreshold` (positive integer) / `sweepThrottleWideMs` (positive number) shape checks; also routed through `validateProviderConfig` as hard errors. "Declared" means `columnDefinitions` PLUS the `calcExpressions` names everywhere — calc columns are real columns to the pull plane (Perspective takes an expression alias anywhere a column goes), so treating them as undeclared would hard-fail legitimate rows. Type and membership checks are SKIPPED where the seam cannot judge: no declared columns at all (schema refined from the first snapshot rows) and weights with no declared `cellDataType`
...
- Bounded writes (2026-07) — `TableWriter` opt `maxRowsPerWrite` (default 25 000) splits an oversized drain into several `table.update()` calls. This bounds write LATENCY, not total work: one 100k-row write pins the worker thread for its whole duration and every window block read queues behind it, so a landing snapshot means a frozen grid — chunking lets reads interleave. Deliberately inert for normal traffic (ordinary ticks are hundreds of rows and take the identical single-write path); it engages during seeding and burst catch-up
- Ingest backpressure telemetry (2026-07) — `TableWriter.getStats(): TableWriterStats` reports `pendingRows` / `bufferedRows` / `writes` / `rowsWritten` / `lastWriteMs` / `maxWriteMs` / `chunkedWrites`; `SsrmDataset` attaches them to every `DatasetStateSnapshot` as an OPTIONAL `ingest: IngestTelemetry` field, so the numbers reach windows over the channel that already exists and NOTHING in the pull plane branches on them. `pendingRows` growing steadily means the feed is outrunning the writer; `maxWriteMs` is the tail that stalls reads. Until this existed, the only way to tell the worker was falling behind was to infer it from a stale-looking grid — which is exactly how the grand-total and group-tick freezes stayed hidden
- `tableSchema` — `schemaFromColumnDefinitions` (config is the single schema/columns declaration) + `refineSchemaFromRows` (first-rows refinement while seeding; discovery mode when no columns declared) + `createRowProjector` (hot-path row projection to the table schema) + `coerceValueToType`/`coerceRowToSchema` (P4b edit coercion: string→float/integer/boolean parsing, Date→ISO on datetime columns, unrepresentable input nulls the cell, unknown fields dropped, `undefined` skipped)
- `classifyFrame` / `matchesEndToken` — pure STOMP frame-body classification (JSON batches / case-insensitive end token / ignore)
- `SsrmControlClient` — window-side control port: `configure` / `restart` / `requestState` / `updateRows(generation, rows)` (P4b: `ssrm-update-rows` — keyed partial rows + the generation they were computed against; the worker refuses stale generations / unkeyed rows / pre-table edits with an error ack and writes nothing, otherwise coerces to the table schema and rides the SAME serialized `TableWriter` path as ingest) + `onState` broadcasts; `ssrmWorkerName(appId, providerId)` (`starui-ssrm:{appId}:{providerId}`); `SSRM_WORKER_ASSET` subpath constant
...
- `toSsrmDatasetConfig(cfg)` — the documented catalog→worker mapping: `StompSsrmProviderConfig` (the `providerType: 'stomp-ssrm'` catalog row) → `SsrmDatasetConfig`; 1:1 on every worker-consumed field, blank optionals dropped (an empty trigger destination publishes no trigger frame; body/headers ride only with a destination); deliberately unmapped: `inferredFields` (editor-side), the reserved `reconnect` block, and EVERY window-side knob (`calcExpressions`/`treePathFields`/`treeParentField`/`weightedAggregates`/`projectDisplayedColumns`/`alwaysProjectColumns`/`wideColumnThreshold`/`sweepThrottleWideMs`) — each is a per-VIEW concern and views are built window-side, so two windows on one provider can legitimately disagree about all of them; the worker hosts ONE table and it stays exactly the declared `columnDefinitions`. Pinned by a test that fails if any of the eight leaks into the `configure` message
