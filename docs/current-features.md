...
- `@starui/ssrm-engine/worker` — `serveSsrmEngineWorker({ openBook, onFault? })` mounts a host on `self.onconnect`: one `SsrmEngine` per book id, N ports per book, retired when the last client sends `close`. `host.publish(bookId, rows, removed?, origin?)` broadcasts a write as the SPARSE patch that was applied, never as whole rows. `SsrmEngineClient.open(port, bookId, { timeoutMs?, onFault?, bookOptions? })` is the window's handle — the same surface asynchronously, with a live mirror of the book size and `subscribe` for pushed writes. The wire is `{id, method, params}` / `{id, ok, result | error}` with one in-flight map per port; an uncloneable result is answered as an error frame rather than dropped, and `messageerror` fails every in-flight call, because a refused structured clone is SILENT at the sender
- 80 tests, including a differential fuzz against a brute-force oracle (250 mutation frames), and the worker path over a real `MessageChannel` — every case there is a way a reply can go missing
