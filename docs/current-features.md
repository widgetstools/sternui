...
- **Attach/replay priority (ADR Phase 0):** while a late-join or refresh replay is in progress, post-ready live delta/`delta-bin`/`delta-patch` fan-out is deferred (last-write-wins per provider) and flushed after replay so ticks cannot interleave with replay chunks; production `installSharedWorkerHub` also enables `deferLiveFanOut` so live fan-out is scheduled on a macrotask — queued `attach` requests can run between ticks instead of waiting behind a saturated sync broadcast loop
...
- **Config SharedWorker (ADR Phase 2):** named `starui-config:{appId}` (`createConfigWorker` / asset `config-catalog-worker.mjs`); `createConfigClient` lazy singleton; RPC ready/get/list/invalidate + `catalog-changed` fan-out; `wireConfigWorkerCatalogSync` keeps the Config SW cache aligned with main-thread ConfigManager writes; `ensureConfigReady({ configWorkerScriptUrl })` / `ensurePlatformReady({ configWorkerScriptUrl })` connect it for P1 tool windows without the data hub. Data hub catalog cache remains for streaming attach (dual invalidate until Phase 2 single-writer follow-up)
- **AppData SharedWorker (ADR Phase 3):** named `starui-appdata:{appId}` (`createAppDataWorker` / asset `appdata-worker.mjs`); `createAppDataClient` lazy singleton; reuses `appdata-*` wire + `AppDataMirror` attach; adds `appdata-lookup` RPC for cross-process template resolution; `ensureConfigReady({ appDataWorkerScriptUrl })` / `ensurePlatformReady({ appDataWorkerScriptUrl })` warm it for P1/P2 profiles. Data hub still keeps in-process `AppDataService` for streaming providers (dual until Phase 4)
- **Provider SharedWorker (ADR Phase 4a–4d + control-plane split):** named `starui-provider:{appId}:{providerId}` (`createProviderWorker` / asset `provider-worker.mjs`); `ProviderHub` scopes monolith attach/delta/replay/idle-stop to one id; `createProviderClient` lazy singleton; `ProviderAppDataLookupCache` prefetches `{{name.key}}` from AppData SW on attach; `ensurePlatformReady({ providerWorkerScriptUrl })` / `DataHubProvider` / `useDataProvider` demux live subscribe and set `hubStreamingDisabled` on the monolith; when Config/AppData clients are present, UI AppData mirror attaches to AppData SW and `ProviderClientAdapter` resolves cfg via Config SW. Monolith hub remains for catalog invalidate dual-path, hub inspector, and legacy non-demuxed callers
