...
- Row grouping (P4a) — group-level requests served from a Perspective `group_by` view on the request's NEXT level (ancestor equality filters pin the route; multi-level verified live); group rows carry the group label, `valueCols` aggregates (sum/min/max/avg/count; unknown aggFuncs reported), leaf child count (`CHILD_COUNT_FIELD` → AG `getChildCount`; key column's `count` aggregate) and a refresh-stable path id (`GROUP_ID_FIELD`, `encodeGroupRowId`); consumers wire `getRowId` via `createSsrmRowIdGetter(keyColumn)`; reads skip Perspective's leading `__ROW_PATH__: []` total row (`num_rows − 1` = group count); `'ag-Grid-AutoColumn'` sorts map to the level's group field (label sort via `unique` aggregate), leaf plans drop it
...
- `StompSsrmProviderConfig` (`providerType: 'stomp-ssrm'`, module `stompSsrm.ts`) — the SSRM pull-plane catalog config: transport (`websocketUrl`/`listenerTopic`/`requestMessage`/`requestBody`/`requestHeaders`/`snapshotEndToken`), **required single-column `keyColumn`** (Perspective table index), `columnDefinitions` (single declaration of table schema + grid columns), `inferredFields`, `tableName`, `heartbeat`, `maxBufferedRows`; window-side knobs (P4b-2, consumed by `createSsrmPullDatasource`/the grid, never the worker): `calcExpressions` (name → Perspective expression), `treePathFields` (serverSide tree hierarchy), `wideColumnThreshold` + `sweepThrottleWideMs` (wide-book delta gate); `reconnect` reserved (documented: the V2 worker never silently redials — restart is explicit). Carries NONE of the push-plane fan-out/throttle/conflation/wire knobs
- `validateStompSsrmConfig(cfg)` — pure structured validation (`StompSsrmValidationIssue[]`: field + code + message): missing/blank `websocketUrl`/`listenerTopic`/`keyColumn`, malformed non-`ws(s)://` URL (skipped for template-token values), composite-array `keyColumn`, `keyColumn` absent from declared `columnDefinitions`; window-side knobs (P5): calc-expression names blank / colliding with declared columns / under the reserved `__ssrm` prefix / cross-referencing another calc alias (engine limit) and blank expressions; tree levels blank / duplicated / absent from declared columns; `wideColumnThreshold` (positive integer) / `sweepThrottleWideMs` (positive number) shape checks; also routed through `validateProviderConfig` as hard errors
