...
- `useProviderDataWiring` — provider→grid hot path inside `MarketsGridContainer`; gates live-tick `applyTransactionAsync` on a combined signal — applies only while the view is visible (`!document.hidden`) AND not paused (the `paused` param, fed by `usePauseCoordinator`). On any transition back to applying it runs one `provider.refresh()` cache replay so the grid catches up in a single `rowData` reset without replaying the intervening storm; on STOMP auto-reconnect (`error` → `ready`) clears the stale banner and triggers `provider.refresh()` so every blotter replays the hub cache without a manual Reload
...
- **Pause / resume realtime updates** — a consumer-side apply-gate (no transport change): while paused, `useProviderDataWiring` stops applying live ticks to the grid (the heavy `applyTransactionAsync` + filter/sort re-run is what starves the UI thread); on resume it calls `provider.refresh()` so the grid catches up in one `rowData` reset — reusing the same path as the `document.hidden` visibility gate, with no data loss. `usePauseCoordinator` (in `MarketsGridContainer`) is a pure reason-set→boolean that stays paused while *any* reason is active: **auto-pauses while the customizer drawer is open** (`MarketsGrid.onCustomizerOpenChange` → `'customizer'` reason) and a manual **"Pause live updates"** `PrimaryToolbar` toggle (`ProviderGridHostApi.paused` / `onTogglePause` → `'manual'` reason). The boolean feeds `useProviderDataWiring`'s `paused` param
...
- `useGridContextLink` — grid-to-grid context linking over colored "Link" groups: publishes the selection and filters rows on peer selections. Echo suppression keys on a **per-window** source id (`makeSourceId` → OpenFin `uuid/name`), so two instances of the same view don't drop each other's broadcasts. Two modes: `'rowId'` (default) broadcasts AG-Grid `getRowId` values (`node.id` = `composeRowId` over the provider key fields) and applies them as an external filter; `'fields'` broadcasts **key columns + values** (the fields that compose `getRowId`) and applies a per-column set-filter — a selected **group expands to its `allLeafChildren`** so any mix of groups/sub-groups/rows resolves to precise leaf-row keys. Receivers apply only the columns they own (`api.getColumn`), merged with the user's manual filters. `onPublish`/`onReceive` callbacks drive notifications. **Large-selection responsiveness:** the publish side **debounces** `selectionChanged` (trailing edge, `publishDebounceMs`, default `50`) so a drag-select broadcasts once instead of per intermediate selection; the `'rowId'` receive side uses a stateful external-filter controller (`createRowIdExternalFilter`) that installs the predicate pair **once** and then only swaps the matched-id `Set` (O(1) membership) + fires one **animation-suppressed** `onFilterChanged()` per context — no per-context grid-option churn. `GridContextLinkConfig`: `enabled`, `mode`, `publish`, `receive`, `publishDebounceMs`, `rowIdField` (auto-filled by `HostedMarketsGrid` from the provider `keyColumn`), `resolve`, `buildContext`, `contextType`, `notify`. Exported helpers: `buildSelectionContext`, `defaultGridLinkResolver`, `applyGridLinkContext`, `createRowIdExternalFilter`, `GRID_LINK_CONTEXT_TYPE`, `normalizeRowIdField`. See [`docs/OPENFIN_GRID_LINKING.md`](./OPENFIN_GRID_LINKING.md)
