...
#### SSRM pull plane (`@starui/ssrm-grid/pull`) — V2, P2–P4b
...
- `StompSsrmProviderConfig` (`providerType: 'stomp-ssrm'`, module `stompSsrm.ts`) — the SSRM pull-plane catalog config: transport (`websocketUrl`/`listenerTopic`/`requestMessage`/`requestBody`/`requestHeaders`/`snapshotEndToken`), **required single-column `keyColumn`** (Perspective table index), `columnDefinitions` (single declaration of table schema + grid columns), `inferredFields`, `tableName`, `heartbeat`, `maxBufferedRows`; `reconnect` reserved (documented: the V2 worker never silently redials — restart is explicit). Carries NONE of the push-plane fan-out/throttle/conflation/wire knobs
