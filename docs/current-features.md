...
- **Provider SharedWorker (ADR Phase 4a–4c):** named `starui-provider:{appId}:{providerId}` (`createProviderWorker` / asset `provider-worker.mjs`); `ProviderHub` scopes monolith attach/delta/replay/idle-stop to one id; `createProviderClient` lazy singleton + `subscribe` via `SharedWorkerDataServicesClient`; `ProviderAppDataLookupCache` prefetches `{{name.key}}` into a sync lookup (hub `appDataLookup` override); `resolveProviderConfigFromConfigClient` for Config SW cfg-free attach; `ProviderClientAdapter({ providerWorker })` opt-in demux away from the monolith hub. Demos still default to the monolith hub; hub shrink is a follow-up
