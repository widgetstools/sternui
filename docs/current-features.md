...
- **Provider SharedWorker (ADR Phase 4a–4b):** named `starui-provider:{appId}:{providerId}` (`createProviderWorker` / asset `provider-worker.mjs`); `ProviderHub` scopes monolith attach/delta/replay/idle-stop to one id; `createProviderClient` lazy singleton + `subscribe` via `SharedWorkerDataServicesClient`; `ProviderAppDataLookupCache` prefetches `{{name.key}}` into a sync lookup (hub `appDataLookup` override); `resolveProviderConfigFromConfigClient` for Config SW cfg-free attach. Demos still use the monolith hub; façade cutover is a follow-up
