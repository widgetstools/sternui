...
- `createSsrmPullDatasource({ connection, keyColumn, columns?, quickFilterColumns? })` — AG `IServerSideDatasource`: rowCount semantics from DatasetState never inferred (`seeding` delivers a flat root store without finalizing + reopens the lazy count; grouped/child stores always finalize per-load with the view count — AG #28 forbids `setRowCount` under grouping; `live`/`empty` finalize, `error` fails); generation fencing drops stale responses; viewport block LRU + serve-then-refresh (cache hit answers synchronously, fresh read lands via index-addressed route-aware `applyServerSideRowData`); ticks refetch EVERY cached block of the current generation (flat, group-level, expanded-leaf) and patch via `applyServerSideTransactionAsync({ route, update })` keyed by `keyColumn` (leaves) / group path id (group rows) — throttled bare `on_update` on a root-route view, never purges; seeding→live refreshes loaded blocks without purge
