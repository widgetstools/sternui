...
- **Pause / resume realtime updates** — `IDataProvider.pause()` / `resume()` / `isPaused()` (hub-level, per-subscriber): the SharedWorker hub stops fanning data deltas to a paused subscriber (status still flows; the cache stays warm), and resume replays the cache once as a consolidated `replace` so the grid catches up in a single batch — keeps the UI thread free under high-frequency update storms with no data loss. `usePauseCoordinator` (in `MarketsGridContainer`) reason-counts pause sources so the stream stays paused while *any* is active: **auto-pauses while the customizer drawer is open** (`MarketsGrid.onCustomizerOpenChange` → `'customizer'` reason) and a manual **"Pause live updates"** `PrimaryToolbar` toggle (`ProviderGridHostApi.paused` / `onTogglePause` → `'manual'` reason). Pause survives provider reconnect (re-asserted on the new subId). Exposed on `useDataProvider` as `pause` / `resume` / `paused`
