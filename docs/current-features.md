...
- `SsrmEngineMarketsGridSurface` — **fourth row-supply surface**, peer to the three above, reached with `rowModel="ssrm-engine"` and an `ssrmEngineClient` (the window's handle on a book held by `@starui/ssrm-engine` in a SharedWorker). Everything with a rule behind it lives in `createSsrmEngineRowEngine`; this is the mount plus the four bridges that can only be built where AG's API is, each of which was a SEPARATE bug on the Perspective path: the quick search (AG's `quickFilterText` is a client-side-row-model option and does nothing here), set-filter value lists (AG builds them from the rows the client holds, which is one block, and a bare `filter: true` handed nothing throws `values is not iterable`), the committed cell edit, and the transaction applier smart edit / bulk update / history undo-redo hand their patches to. `getRowId` is the group PATH, the tree marker, or AG's own grand-total id. It uses the SAME shared modules the Perspective surface does rather than copies. **The quick-search bridge listens on `modelUpdated` AND reconciles on the viewport timer**, because MEASURED: with a term that matches nothing, clearing the box fires NO event at all — not `modelUpdated`, `filterChanged`, `storeUpdated` or `gridOptionChanged` — so the box was unclearable from an empty grid, which is the state a user most needs to escape. It puts THREE optional seams on the grid `context` beyond the shared ones, each declared rather than sniffed: `ssrmCountMatchingExpression` (StarUI source, parsed HERE with `tokenize`/`parse` into the same AST calculated columns already send, because a closure is not structured-cloneable and there must not be a second parser), `ssrmAggregateScalar` (which measures the WHOLE BOOK, dropping the filter model and quick filter — Excel's convention, so a threshold does not move when a user filters, at the known cost that such a rule can disagree with the average in the totals row on the same screen), and `ssrmCalcDiagnostics` (what the engine made of each authored calculated column, so an author is told WHY one came back blank). `ssrmExpressionDialect: 'starui'` is declared because the two dialects are both plain strings and a mismatch is silent — the worker would refuse every rule and the header would simply never light, which is indistinguishable from a rule nothing matches. An engine that cannot answer one of these OMITS it, and a caller paints nothing rather than reading absence as "no row matches"
...
- **Calculated columns** — virtual cols from expressions. On a SERVER-side surface the editor also shows `useServerCalcDiagnostics(colId)` — what the ENGINE made of the expression, read back through the optional `ssrmCalcDiagnostics` seam on the grid context: `compile` (ENGINE REFUSED — the column is not installed and reads its field binding), `runtime` (FAILED WHILE EVALUATING, with a hit count) and `column` (UNKNOWN FIELD — not an error; every row reads null, which is the quietest way a calculated column goes wrong). This exists because the planner only ever answers "does this PARSE", by design — the engine holds the refusal list and a second copy in `@starui/grid` would be a second thing to keep in step — so everything the engine refused used to arrive as a blank column with no explanation, and `console.warn` in a SharedWorker reaches no console anywhere. An absent seam renders NOTHING rather than "no problems found", which is a claim CSRM, a not-yet-attached engine and an engine without such a list all cannot support
...
- `SharedWorkerDataServicesClient.attachPerspective(providerId)` — binds this window to a provider's Perspective Table: transfers a fresh `MessagePort` to the worker, which starts the provider from its catalog row if it is not already running (a blotter opens on its own, without needing a push subscriber first — the row is resolved **on demand** via `ConfigCatalogCache.ensure`, so a window that saves its provider config and attaches straight after does not lose the race against the async `wireWorkerCatalogSync` invalidate) and binds a ProxySession to the port. Resolves `{ ok: true, port, tableName }` for the window's Perspective `Client` to speak over, or `{ ok: false, reason }` — a value, not a rejection — when the provider holds no Table (wrong provider type, a worker built without the Perspective loader, or a `keyColumn` that cannot index one), so a caller falls back to the push path immediately. The reply WAITS for the Table to exist (`feed.whenReady()`): `open_table(name)` resolves for a name the engine does not hold yet rather than throwing, and a window handed that handle reads 0 rows forever while every later attach reads the full book. With a declared schema the wait is immediate. The unused port is closed on failure and on `client.close()`
- `SharedWorkerDataServicesClient.attachSsrm(providerId)` — the peer of `attachPerspective`, for a provider's **SSRM book**: transfers a fresh `MessagePort`, the worker's engine host serves RPC on it, and the reply waits for the book to exist. Resolves `{ ok: true, port, bookId }` for `SsrmEngineClient.open(port, bookId)`, or `{ ok: false, reason }` — a value, not a rejection — when the worker was built without an SSRM loader or the provider's `keyColumn` cannot index a book. No rows cross on attach: the window pulls the blocks its viewport asks for and is pushed the cells that tick
- `./runtime/ssrm` — `createSsrmBookFeed({ keyColumn, declaredSchema?, createBook, buildAfterRows?, onDiagnostic? })` fills a book by DECORATING `ProviderEmit`, so no transport changes and any provider drives it; `createSsrmHost({ loadSsrm, onError? })` owns the engine and the named books and binds attached ports. The engine is **injected** by the worker entry, not imported — `@starui/host-data` must not depend on a `react-grid` package, and injection is what lets the book live in the worker the provider's rows are already in. Hub option `loadSsrm`; `inferSsrmSchema(rows, keyColumn)` for providers that declare no columns, which reads EVERY value rather than sampling (sampling row types truncated 19,999 of 20,000 values on the Perspective path) The emit SEQUENCE is fuzzed rather than exampled: 260 random interleavings of empty replaces, flagged and unflagged chunks, a restart landing mid-snapshot and live deltas, asserted against one independent rule — a `replace` clears the book and every batch after it upserts. Reintroducing the rule-8 bug (deciding snapshot-vs-update inside the queued work instead of synchronously in `emit`) turns it red at a named seed with the event script printed.
- **The book is fed where the feed already is.** A provider-fed book lives in the data-services worker; a generated book lives wherever its generator does. There is no route between two SharedWorkers that does not pass through a window, so hosting a fed book in the other worker would mean forwarding every row **per window** — a second copy of the feed that scales with the thing this path exists to make cheap
- **Never supply `loadSsrm` and `loadPerspective` to the same worker.** Both tee off the same provider, so the worker builds two engines over one feed and every figure taken against it is of two engines recorded as one — the contamination that made a renderer reading on the lab's Stress tab 1,114 MB instead of 411 MB
- **The emit sequence is not what it looks like, and all three consequences were bugs first.** `stomp.ts` emits `{rows: chunk, replace: offset === 0}`, so a snapshot is an EMPTY `replace:true` clear, then a flagged first chunk, then unflagged chunks that are still snapshot. Treating only the flagged chunk as snapshot builds the book from 1,000 rows instead of 20,000; treating an empty `replace` as a no-op leaves a stale book on screen forever when the new one is empty; discarding staged rows only when a book already exists merges an abandoned snapshot into the next one. A fourth, found here: the snapshot-vs-update decision must be made **synchronously in `emit`**, not inside the queued work — `emit` is synchronous and the queue is not, so every chunk was enqueued before the first had run, all of them believed they were the first, and the book ended up holding only the LAST chunk
- **`perspective-ssrm-lab` Stress tab — only the active surface's row supply is subscribed.** `StreamOptions.enabled` / `LabStreamOptions.enabled`: a null `providerId` tells `useProviderStream` not to subscribe, and dropping the subscription now also releases the rows already held. Distinct from `enableUpdates`, which only stops the ticks — the snapshot still arrives. MEASURED, and it cost a renderer: the tab called both supplies unconditionally, so a window whose grid reads from the worker-held Table also held two live arrays of **50,000 rows x 256 fields**, rendered by nothing, and Chrome killed it with "Aw, Snap! Error code: Out of Memory". JS heap 481 MB -> **74 MB** idle, and 470-530 MB -> **56-113 MB** across eight rounds of scroll/group/filter
- `bootWorkerEntry` installs worker-scope `unhandledrejection` / `error` reporting — an unhandled rejection in a SharedWorker is otherwise INVISIBLE (nothing reaches any window's console, the worker keeps running, and whatever awaited the promise never settles, which from a blotter is indistinguishable from a hang). MEASURED motivation: on a 50k x 400 book the Perspective engine throws `DataView.prototype.getInt32 on a detached or out-of-bounds ArrayBuffer` from inside its OWN transport once per worker boot — its protocol buffers are views over the wasm `HEAPU8`, which detaches when wasm memory grows. Zero at 500 rows; chunking the write made it 6 rather than fewer, so the trigger is growth itself. Not fixable this side of the engine; this makes it attributable
...
- `startMockPerspective(cfg, emit, { perspectiveHost, tableName?, onDiagnostic? })` — the generated mock book delivered through a Perspective Table. Stands to `startMock` exactly as `startStompPerspective` stands to `startStomp`, and is the same thin composition: the generator, tick behaviour and restart semantics stay in one place and the Table is a side effect of the emit stream, forwarded synchronously and unmodified so the push path is untouched. Exists so the Perspective row engine can be driven with **no broker**, which is what lets an app and its CSRM twin run the same columns and profiles over the same data — a difference between them is then the engine and nothing else. **`rowShape` defaults to `'ssrm'` here, unlike `mock`:** a Perspective schema is a flat map of typed columns and the mock positions row is deeply nested, so emitting it unflattened would hand the feed nested objects that `observeRows` drops — a Table with a fraction of its columns and no error anywhere. A config with `rowShape: 'csrm'` or no `columnDefinitions` is refused loudly (`index-invalid` diagnostic) rather than served short, as is a composite `keyColumn`. An override registered via `registerProvider` still wins. 10 tests
...
| `perspective-ssrm-lab` | **The feature lab on the Perspective pull path** — `markets-grid-lab` cloned onto `rowModel="perspective"` across every tab, fed by a `mock-perspective` provider so it needs no broker. Same 44 columns, ~150 profile JSONs, seeds, guides and all 28 demo scenarios, so running it beside the CSRM lab makes any difference attributable to the engine alone. One Table per tab (each tab keeps its own `providerId`, so the per-tab row counts and updates-off tuning survive). The header engine toggle is gone — the engine is a property of the app. A tab renders a placeholder until its Table exists, because exactly one grid may mount per `GridPlatform`. A demo scenario becomes an EDIT to the shared Table rather than a client-side overlay, so peer windows see it; clearing one is a provider restart. The Stress tab (20,000 rows x 120 real columns) is ONE surface with a runtime switch: `?engine=ssrm` runs the same book on `@starui/ssrm-engine` in a SharedWorker (`src/workers/ssrmBookWorker.ts`, book contents in `src/data/stressBook.ts` — which may not import React, AG Grid or anything DOM-coupled) instead of the Perspective pull path, so the probes apply unchanged and a comparison is of engines alone. The two surfaces are separate components on purpose: `useLabPerspectiveRows` seeds and attaches as a side effect of being called and a hook cannot be conditional, so while they shared one component `?engine=ssrm` built the whole Perspective Table as well and every renderer figure taken there was of two engines. **A dedicated `SSRM Engine` tab** (`src/tabs/SsrmEngineTab.tsx`, Performance category) is the discoverable showcase for `@starui/ssrm-engine`: the same worker-held 20k × 120 book on a plain `AgGridReact`, with four calculated columns installed unconditionally and toolbar buttons that SORT, FILTER and GROUP by them in one click. The buttons do the thing rather than asking the viewer to find a header menu, because the capability is invisible both ways — before session 5 a sort or filter on a calculated column was a silent no-op, so a demo that requires noticing an absence is not a demo. A stats strip reports book size, rows AG displays, the LAST block read, calculated-column refusals (empty is the good case, and `console.warn` in a SharedWorker reaches no console anywhere) and rows pushed. It reports the last block rather than a median deliberately: every block this tab samples is COLD, since each button changes the query shape, so a median implies a steady state the tab never has — the first draft showed 62.90 ms against a documented 2.40 ms purely because n was 1. Expressions and colDefs come from `src/data/ssrmCalcColumns.ts`, shared with the flag below so the two cannot drift. It mounts a plain `AgGridReact` on purpose, and keeps doing so: it is the CONTROL for the tab below. **A second `SSRM Engine · MarketsGrid` tab** (`src/tabs/SsrmEngineMarketsGridTab.tsx`) runs the SAME book, in the same SharedWorker, under the same book id — so the two tabs share one book — with the whole MarketsGrid platform on top (`rowModel="ssrm-engine"`). Two tabs rather than a toggle inside one, because the A/B that answers "what does the platform cost the read path" has to ALTERNATE two addresses in one series and a toggle gives a probe nothing to interleave with; both publish the same `__ssrmEngineGrid` handle, so `browserSmokeProbe` and `workerBoundaryProbe` take `--tab` and are otherwise unchanged. Its calculated columns are seeded as CUSTOMIZER state rather than passed as a prop, so they travel the path a user's own column takes — module → planner → AST → worker — and are sorted, filtered and grouped from the grid's own header menus and row-group panel, with no demo buttons. Four dimension columns carry a real `agSetColumnFilter` here and nowhere else in the lab, because engine-backed set-filter values are the capability this surface adds and a tab that quietly kept text filters would prove nothing. **`&calc=1`** on the Stress tab additionally installs the same four calculated columns in the worker — authored as STRINGS and run through the real `planSsrmCalcColumns(..., { backend: 'ssrm-engine' })`, so the path exercised is the one a user takes rather than a hand-built AST that would skip `tokenize`/`parse` and the planner. OFF by default because the block round trip is a documented figure and four calculated columns change what a block carries; both numbers are worth having. The grid does not mount until `setCalcColumns` has landed and `calcDiagnostics()` has been read back onto `__ssrmCalcDiagnostics` and the console — a refused column is a column of blanks, and `console.warn` in a SharedWorker reaches no console anywhere (port 5301). **Session 8's bake-off lives on this tab**: `?engine=ssrm&surface=marketsgrid` mounts the ssrm MarketsGrid surface on the SAME `gridId`, seeded profile, column defs and grid options the Perspective branch uses, so the only difference between the two is `rowModel` — every earlier ssrm-vs-Perspective figure had compared a plain `AgGridReact` against a MarketsGrid and reported the sum of a row supply and a platform as an engine difference. The book is back to **50,000 rows** for it, reversing a cut that had been made because Perspective was dying of memory at that size; the book id is now keyed by the row count, because the worker memoises a book per id and would otherwise hand a window the previous size silently |
| `ssrm-markets-grid-lab` | **The dedicated lab for the engine that SHIPS.** MarketsGrid on `rowModel="ssrm-engine"` over a 50,000 x 120 book generated in the app's own SharedWorker — one engine, one grid, no tab strip and no `?engine=` parameter deciding what is mounted. It exists because the bake-off lab presents a settled question as open: its PERFORMANCE section offers "Stress Test" (two engines behind a query parameter), "SSRM Engine" (a bare `AgGridReact` control) and "SSRM Engine · MarketsGrid" (the product), with nothing on screen saying which engine is on and the one to open carrying the longest name. Two of those three serve a comparison that finished in session 8. ONE seeded profile, and every piece of it is a thing that breaks in its own way on a server row model: six calculated columns authored in the customizer (so their ASTs cross the worker port and they are sortable and groupable rather than merely displayed), a whole-book style rule `[esgScore] > 999` matching ~1 row in 1,000 so that NO loaded block satisfies it — its header indicator can only light if the worker was asked about the book, which is the one visible thing here a client-side grid physically cannot do — a timed flash rule written as `value != null` (the spelling that was dead until the timed evaluator stopped binding `value` to null), and BOTH totals rows, since a grand total is created one way and updated another while a group footer is a node `forEachNode` does not traverse at all. No `@finos/perspective` dependency: the whole bundle is smaller than the bake-off lab's 5,070 kB inline Perspective chunk alone. No data-services hub either — the book is generated, not provider-fed, so nothing can fail before the first render. Its SharedWorker is named `starui-ssrm-marketsgrid-lab` rather than the other lab's `starui-ssrm-book`, because a SharedWorker is keyed by script URL AND name and sharing one would put two books in one worker. Publishes the same `__ssrmEngineGrid` handle every ssrm probe reads, tagged `surface: 'ssrm-markets-grid-lab'` so a probe landing on the wrong app says so. Port 5321, `strictPort` on both dev and preview, because measuring somebody else's stale build is indistinguishable from measuring your own. `playwright.ssrm-lab.config.ts` builds and serves it, and `e2e/ssrm-marketsgrid.spec.ts` (6 tests) is the spec session 6 recorded as missing. **`?rows=` and `?tick=`** size the book and set its tick rate at runtime, defaulting to 50,000 / 200 ms: the book ID CARRIES the row count (`stress-500000`), because the worker memoises a book per id and hands the existing one to the next client that asks while IGNORING its options — a fixed id serves the previous size to a window that asked for 500,000, silently, with a plausible row count on screen. Both are resolved in the window and passed to the worker, never read inside it: a SharedWorker has a `location` too, but it is the WORKER SCRIPT's URL, so the same read there would build the default book while the header reported the size the reader asked for. They do not reintroduce the `?engine=` problem this app was built to avoid — that switch changed which ENGINE was mounted while the screen looked identical; these change the size of the one book, and the size is printed in the header, carried in the book id and reported on the measurement handle |
