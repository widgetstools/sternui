...
**Provider SharedWorker (ADR Phase 4a–4d + control-plane + single-writer):** Pass Config + AppData + provider worker URLs to `ensurePlatformReady`. Live subscribe goes to per-provider workers; monolith rejects streaming (`hubStreamingDisabled`) and skips AppData (`hubAppDataDisabled`); catalog invalidate + React list/config hooks prefer Config SW when wired. Hub inspector shows a topology note when demuxed.
