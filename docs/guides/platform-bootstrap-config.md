...
**Provider SharedWorker (ADR Phase 4a–4d + control-plane split):** `createProviderClient({ appId, providerId, workerScriptUrl })` spawns `starui-provider:{appId}:{providerId}` (Vite: `import providerWorkerUrl from '@wellsfargo-starui/host-data/assets/provider-worker.mjs?url'`). Pass `providerWorkerScriptUrl` (and ideally Config + AppData worker URLs) to `ensurePlatformReady` so live subscribe goes to per-provider workers, the monolith rejects streaming attach (`hubStreamingDisabled`), UI AppData mirror attaches to AppData SW, and catalog cfg prefers Config SW. The thin monolith hub remains for catalog invalidate dual-path / inspector / legacy callers.
