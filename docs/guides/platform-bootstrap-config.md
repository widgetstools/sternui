...
**Provider SharedWorker (ADR Phase 4a–4d):** `createProviderClient({ appId, providerId, workerScriptUrl })` spawns `starui-provider:{appId}:{providerId}` (Vite: `import providerWorkerUrl from '@wellsfargo-starui/host-data/assets/provider-worker.mjs?url'`). Pass `providerWorkerScriptUrl` (and ideally `appDataWorkerScriptUrl`) to `ensurePlatformReady` so `getProvider` / `useDataProvider` route live subscribe to per-provider workers. Provider workers prefetch `{{…}}` from the AppData SW on attach. The monolith `mkt-data-services` hub remains for catalog + AppData mirror until a later cleanup removes unused multi-provider hosting.
