...
Submenu items already used `contentMenuIcon()` with theme-aware SVG data URLs (fixed **slate** strokes via `dockMenuIconStrokeColors()` — see [openfin-starui-theme-bridge.md](./openfin-starui-theme-bridge.md)). Root folders did not, because the platform API and UI library omit folder icons.
