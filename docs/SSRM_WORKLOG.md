...
| B14 | ~~Production builds shipped NO Perspective WASM~~ **Fixed — `staruiPerspectiveWasmAssetsPlugin`.** `vite build` copied the `?url`-imported worker `.mjs` files but never their wasm siblings; the psp link then fetched `perspective-server.wasm`, got the SPA-fallback index.html, and died with `WebAssembly.instantiate(): expected magic word … found 3c 21 64 6f` (`<!do`) — every production pull-plane app was broken (dev worked only because the dev server reads host-data's dist). The shared consumer Vite config now emits `perspective-server.wasm` + `perspective-js.wasm` unhashed next to the emitted worker asset whenever an app bundles the pull plane. Found while measuring production scroll feel — which settled the dev-tax question: fling p95 **50 ms in production vs 183–250 ms in dev** (React dev instrumentation is ~4–5× of the residual fling cost; `React.createElement` disappears from the production profile entirely) | `scripts/staruiConsumerAliases.mjs` |
| B13 | ~~Blank blotter on data-less cold start~~ **Fixed — attach retry + configure re-arm.** Opening a pull blotter while the provider had no rows (feed down/restarting) failed the schema-less psp link retryably ("no schema available… retry once data exists") — but nothing retried, the table was never created, the open-only configure exhausted its retries, and the grid sat blank with "Rows: …". `useSsrmPullEngine` now listens for the attach ack and re-posts the hand-off with backoff (2s→30s cap) until linked; `useSsrmGridController` re-arms configure every 5 s after fast retries are spent. The blotter self-heals the moment the feed comes alive | `useSsrmPullEngine.ts` / `useSsrmGridController.ts` |
