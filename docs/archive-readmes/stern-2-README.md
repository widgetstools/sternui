...
Every package and app from this repo now lives under `@starui/*` in
...
|---|---|
| `packages/widget-sdk` | [`packages/widget-sdk`](https://github.com/widgetstools/marketsui-platform/tree/main/packages/widget-sdk) — `@starui/widget-sdk` |
| `packages/widgets` (React blotter/chart/heatmap) | [`packages/widgets-react`](https://github.com/widgetstools/marketsui-platform/tree/main/packages/widgets-react) — `@starui/widgets-react` |
| `packages/openfin-platform` | [`packages/openfin-platform-stern`](https://github.com/widgetstools/marketsui-platform/tree/main/packages/openfin-platform-stern) — `@starui/openfin-platform-stern` |
| `packages/shared-types` | Merged into [`packages/shared-types`](https://github.com/widgetstools/marketsui-platform/tree/main/packages/shared-types) — `@starui/shared-types` |
| `apps/server` | [`apps/config-service-server`](https://github.com/widgetstools/marketsui-platform/tree/main/apps/config-service-server) — `@starui/config-service-server` |
...
- **`StompDataProvider`** — the concrete `IDataProvider` implementation
  ...
  data-plane work. Unchanged, relocated under `@starui/widgets-react`.
- **OpenFin platform bootstrap** — preserved in
  `@starui/openfin-platform-stern` alongside the markets-ui flavour
  in `@starui/openfin-platform`. Platform shells are the only layer
  allowed to import `@openfin/core`.
- **Widget registry + launch spec** — `@starui/widget-sdk` keeps the
...
3. Rename imports: `@stern/widget-sdk` → `@starui/widget-sdk`;
