...
| **4** | Extract **per-provider** SharedWorkers + façade; shrink/remove monolith hub | **In progress (4a–4c):** provider SW + AppData lookup cache + Config bridge; `ProviderClientAdapter({ providerWorker })` routes subscribe to `starui-provider:{appId}:{id}` (opt-in). Monolith hub still default for demos; hub shrink is 4d. |
