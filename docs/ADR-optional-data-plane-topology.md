...
| **4** | Extract **per-provider** SharedWorkers + façade; shrink/remove monolith hub | **In progress (4a+4b):** `starui-provider:{appId}:{providerId}` (`ProviderHub` / `createProviderClient` / `provider-worker.mjs`); `ProviderAppDataLookupCache` + hub `appDataLookup` override for sync `{{…}}` across process; `resolveProviderConfigFromConfigClient`. Monolith hub still serves demos; façade flag + hub shrink are follow-ups (4c–4d). |
