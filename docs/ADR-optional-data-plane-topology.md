...
| **2** | Extract **Config** SharedWorker + `createConfigClient`; retarget invalidate | **Done (single-writer path):** Config SW + `createConfigClient`; when Config SW is wired, hub `wireWorkerCatalogSync` is skipped and React catalog hooks / `DataProviderConfigStore` invalidate prefer Config SW. |
| **3** | Extract **AppData** SharedWorker + mirror attach | **Done (single-path for demuxed apps):** AppData SW + mirror; when AppData SW URL/client is present, monolith sets `hubAppDataDisabled` (no hydrate/serve); provider workers skip in-process AppData hydrate when the AppData bridge is connected. |
| **4** | Extract **per-provider** SharedWorkers + façade; shrink/remove monolith hub | **Done (4a–4d + control-plane + inspector note):** demux + `hubStreamingDisabled` + AppData/Config authorities; hub introspect exposes `streamingDisabled` / `appDataDisabled` / `topologyNote` for demuxed apps. Full hub deletion still a follow-up. |
