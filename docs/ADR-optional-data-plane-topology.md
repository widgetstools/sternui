...
| **4** | Extract **per-provider** SharedWorkers + façade; shrink/remove monolith hub | **Done (4a–4d + control-plane split):** provider SW + AppData bridge on attach; `ensurePlatformReady({ providerWorkerScriptUrl })` → demux + `hubStreamingDisabled`; UI AppData mirror prefers AppData SW; adapter catalog prefers Config SW when clients are present. Monolith retained for catalog invalidate dual-path, hub inspector, and non-demuxed callers (full delete / single-writer still follow-ups). |
