...
**Persistence on workspace restore.** On the next workspace load, `OpenFinRuntime` reads `customData.savedTitle` from the resolved view options during construction and reapplies it to `document.title`. Without this step the rename would be lost on every reload — `document.title` is a runtime-only DOM mutation that the snapshot never captures. The mechanism mirrors §1.13 (per-view active-profile override): a small piece of state lives on `customData`, the snapshot captures it for free, and the runtime reapplies it on boot.
