...
- **One row per instance, all layouts bundled.** A grid's entire layout set lives in a single `AppConfigRow` keyed by `(appId, userId, instanceId)`. The row's `payload` is `{ version, profiles: [...], gridLevelData }`. This is intentional — admins opening the **Config Browser** see one row per blotter ("alice's positions blotter, all her saved views in there"), not N rows per view.
...
- **Optimistic concurrency.** Every save reads the row's `version`, writes back `version + 1`, and throws `ProfileSetVersionConflictError` if someone else (another tab, another machine) bumped the version in between. Stops two windows silently clobbering each other.
...
- **Migration helper.** `migrateProfilesToConfigService({ source, target, ... })` is a one-shot admin action that copies layouts out of an old Dexie/memory adapter and into the bundled ConfigService row, with skip-if-exists or overwrite strategies.
...
8. Hours later the user clicks "Save Workspace" on the OpenFin dock. The platform fans out `workspace-saving`; this view's handler awaits `marketsGrid.saveAll()`, then OpenFin captures the snapshot. The snapshot — including `customData.activeProfileId` on every view — lands as a ConfigService `componentType: 'workspace'` row.
