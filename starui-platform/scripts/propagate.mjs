#!/usr/bin/env node
/**
 * propagate.mjs — pack workspace libraries to content-hashed tarballs in
 * `libs/`, mirror version-only copies to `dist/packages/`, then sync apps.
 *
 * Layout
 * ------
 *   libs/starui-grid-0.1.0-<sha8>.tgz   — apps install from here (cache-safe)
 *   dist/packages/starui-grid-0.1.0.tgz  — human-readable mirror (same bits)
 *   libs/manifest.json                  — maps package → hashed + dist filenames
 *
 * Usage
 * -----
 ...
 * Output
 * ------
 *   libs/<scope>-<name>-<version>-<sha8>.tgz   (install target)
 *   dist/packages/<scope>-<name>-<version>.tgz (readable mirror)
 *   libs/manifest.json                          (package → {filename, distFilename, …})
 *
...
function discoverPackages() {
  ...
  return [...dirs]
    ...
    .filter((pkg) => !PACK_EXCLUDE.has(pkg.name));
}
...
/**
 * Pack to `libs/<name>-<version>-<sha8>.tgz` and mirror the same bytes to
 * `dist/packages/<name>-<version>.tgz`.
 */
function packToManifest(pkg, prevEntry) {
  const distFilename = rawPackFilename(pkg);
  if (args.dryRun) {
    const hashedName = distFilename.replace(/\.tgz$/, `-DRYRUN0.tgz`);
    log(`pack: ${pkg.name}@${pkg.version} → libs/${hashedName} + dist/packages/${distFilename} (dry-run)`);
    return {
      filename: hashedName,
      distFilename,
      sha: 'DRYRUN0',
  ...
  log(
    `pack: ${pkg.name}@${pkg.version} → libs/${hashedName} + dist/packages/${distFilename}${
      contentChanged ? '' : ' (unchanged)'
    }`,
  );
  return { filename: hashedName, distFilename, sha, version: pkg.version, contentChanged };
}
...
function writeManifest(manifest) {
  ...
  log(`wrote: dist/packages/manifest.json (${Object.keys(distView).length} entries)`);
}
...
/** True when `appPkgPath` lists `packageName` as a `file:…/libs/*.tgz` dep. */
...
function gcOrphanedDist(manifest) {
  ...
  for (const entry of readdirSync(DIST_DIR)) {
    ...
    if (args.dryRun) {
      log(`gc: would remove dist/packages/${entry}`);
    } else {
      unlinkSync(path);
...
function main() {
  ...
  for (const pkg of targets) {
    ...
    const result = packToManifest(pkg, prev);
    ...
    updates[pkg.name] = { ...manifest[pkg.name], contentChanged: result.contentChanged };
  }

  writeManifest(manifest);

  // Sync every apps/* package.json that references one of the updated
  // packages (filename migration), then reinstall in each app where
  // something actually changed — including same-filename content updates.
