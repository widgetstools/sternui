...
[`apps/legacy/e2e-openfin-vitest/`](../apps/legacy/e2e-openfin-vitest/).
...
```bash
# Local — spawns apps/e2e/openfin-workspace dev server on :5181
...
1. Playwright's `webServer` block boots
   [`@starui/e2e-openfin-workspace`](../apps/e2e/openfin-workspace/) on
...
browser harness uses against `apps/e2e/browser-blotter`. Helpers from
