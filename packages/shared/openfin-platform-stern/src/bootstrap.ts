 ...
 *
 * Built on the unified `@starui/config-service` ConfigClient. Swapping
...
// Intentionally local to this file (not imported from @starui/config-service)
...
// from `@starui/widgets-react` and called `.configure({ apiUrl })`.
// `dataProviderConfigService` has since moved to `@starui/data-plane`,
...
import { createMenuItem } from '@starui/shared-types';
