/**
 * StompProbe — snapshot fetching and field inference.
 *
 * Distinct from `StompStreamProvider` (a continuous-stream Provider for the
 * data-services runtime). This class is for one-shot snapshot retrieval used by
...
import { composeRowId, type FieldInfo } from '@starui/shared-types';
...
export class StompProbe {
  ...
  static inferFields(
    ...
    working.forEach(row => StompProbe.inferObject(row, '', fields));
  ...
  private static inferObject(obj: any, prefix: string, fields: Record<string, FieldInfo>): void {
    ...
    Object.entries(obj).forEach(([key, value]) => {
      ...
      if (!fields[path]) {
        fields[path] = {
          path,
          type: StompProbe.inferType(value),
        ...
        if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
          ...
          StompProbe.inferObjectChildren(value, fields[path].children!, path);
        }
      } else {
    ...
    Object.entries(obj).forEach(([key, value]) => {
      ...
      if (!children[key]) {
        ...
        if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
          ...
          StompProbe.inferObjectChildren(value, children[key].children!, fullPath);
        }
      } else {
        if (value === null || value === undefined) {
