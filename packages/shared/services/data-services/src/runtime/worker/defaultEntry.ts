/**
 * Default SharedWorker entry for `@starui/data-services`.
 *
...
async function boot(): Promise<void> {
  ...
  console.info(
    `[@starui/data-services worker] ConfigManager initialised (mode: ${configManager.isRestMode() ? 'REST' : 'local'})`,
  );
  await installSharedWorkerHub({ configManager });
  // eslint-disable-next-line no-console
  console.info('[@starui/data-services worker] booted; hub waiting for ports');
}

boot().catch((err) => {
  // eslint-disable-next-line no-console
  console.error('[@starui/data-services worker] boot failed', err);
