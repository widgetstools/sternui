...
```ts
import { SideCellRenderer } from '@starui/design-system/cell-renderers';
```
...
- **Relative import paths** (`../../../packages/design-system/src/themes/fi-dark.css`) are intentional — they resolve correctly under the npm workspace symlinks used by every app here. A package-path import (`@starui/design-system/themes/fi-dark.css`) would work too once exports are declared, but the relative path matches every existing app for consistency.
