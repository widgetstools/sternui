...
.gc-flash-pulse { animation: gc-flash-pulse var(--flash-period, 1s) infinite ease-in-out; }
.ag-header-cell.gc-flash-hdr-pulse { animation: gc-flash-pulse var(--flash-period, 1s) infinite ease-in-out; }
`;
...
function buildCssText(
  ...
  if (pulse?.enabled && (pulse.target === 'cells' || pulse.target === 'cells+headers' || pulse.target === 'row')) {
    lines.push(`${cls} { animation: gc-flash-pulse var(--flash-period, 1s) infinite ease-in-out; }`);
  }
