...
export const TitleInput = forwardRef<HTMLInputElement, TitleInputProps>(function TitleInput(
  ...
  return (
    <input
      ref={ref}
      className={[
        'flex-1 min-w-0 h-8 px-2.5 border border-border rounded-sm bg-background text-foreground font-sans font-semibold outline-none transition-colors duration-[120ms] focus:border-primary placeholder:text-muted-foreground placeholder:font-medium text-[length:var(--ds-font-size-sm)] tracking-tight',
        className ?? '',
      ].join(' ')}
      style={style}
      {...rest}
