...
export function UserProfileEditor() {
  ...
  function buildNextRow(): UserProfileRow {
    return {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        await guardOptimisticUpdate<UserProfileRow>({
          expectedUpdatedTime,
          fetchCurrent: () => client.userProfiles.get(editingId),
        });
  ...
  async function handleReload() {
    ...
    if (!editingId) return;
    const fresh = await client.userProfiles.get(editingId);
    if (fresh) {
  ...
  const columns: EditorTableColumn<UserProfileRow>[] = [
    {
      key: 'userId',
      header: 'User ID',
      cell: (r) => <span className="font-mono">{r.userId}</span>,
    ...
    },
    {
      key: 'appId',
      header: 'App',
      cell: (r) => <span className="text-muted-foreground">{r.appId}</span>,
      sortValue: (r) => r.appId,
      width: '12rem',
    },
    {
      ...
      <EditorShell
        ...
        list={
          <EditorDataTable
            ...
            rowKey={(r) => r.userId}
            onEditRow={startEdit}
            emptyMessage="No user profiles defined."
