    ...
    theme: [
      {
        ...
        palettes: {
          ...
          light: {
            brandPrimary: theme?.brandPrimary ?? "#0A76D3",
            // brandSecondary is scheme-specific in OpenFin's reference
            // palettes (dark #383A40 / light #DDDFE4). Reusing the dark
            // value here makes OpenFin derive a dark secondary-button text
            // over a dark fill, so the Cancel button in workspace dialogs
            // renders "blacked out" in light mode (text only visible on
            // hover). Pin OpenFin's light secondary so text stays legible
            // — same reason backgroundPrimary is hardcoded light below.
