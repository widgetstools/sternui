...
type DrawerContentProps = React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Content> & {
  ...
  /** E2E hook for the modal backdrop. */
  overlayTestId?: string;
  /** Hide the bottom drag handle (default for side drawers). */
  hideHandle?: boolean;
};
...
const DrawerContent = React.forwardRef<
  ...
  DrawerContentProps
>(({ className, children, overlayProps, overlayTestId, hideHandle = false, ...props }, ref) => {
  const portalContainer = useResolvedPortalContainer();
  return (
    <DrawerPrimitive.Portal container={portalContainer}>
      <DrawerOverlay data-testid={overlayTestId} {...overlayProps} />
      <DrawerPrimitive.Content
        ref={ref}
        className={cn(
          'group/drawer-content fixed z-[11000] flex h-auto flex-col border bg-background',
