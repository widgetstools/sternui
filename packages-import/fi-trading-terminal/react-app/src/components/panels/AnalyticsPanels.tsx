...
const SECTOR_COLORS = ['#1e90ff', '#00bcd4', '#c084fc', '#0ecb81', '#f0b90b', '#f59e0b'];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bn-bg1)', overflow: 'hidden' }}>
      <div style={{ flex: 1 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={BUCKET_DATA} margin={{ top: 12, right: 16, bottom: 4, left: 4 }}>
            ...
            <Tooltip content={<TT />} />
            <Bar yAxisId="dv01" dataKey="dv01" name="DV01" fill="#1e90ff" radius={[3, 3, 0, 0]} barSize={18} fillOpacity={0.8} />
            <Bar yAxisId="oas" dataKey="avgOas" name="Avg OAS" fill="#f0b90b" radius={[3, 3, 0, 0]} barSize={14} fillOpacity={0.7} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      {/* Summary footer */}
      <div style={{ display: 'flex', gap: 14, padding: '6px 14px', borderTop: BD, flexShrink: 0 }}>
        {[
          ...
          { l: 'Wt Avg OAS', v: `+${Math.round(BUCKET_DATA.reduce((a, d) => a + d.avgOas * d.bonds, 0) / BUCKET_DATA.reduce((a, d) => a + d.bonds, 0))}bp`, c: '#f0b90b' },
        ].map(s => (
            ...
            <Tooltip content={(props: any) => {
              ...
              return (
                <div style={{ background: 'var(--bn-bg2)', border: BD, borderRadius: 3, padding: '6px 10px', fontFamily: 'JetBrains Mono,monospace', fontSize: 11 }}>
                  ...
                  <div style={{ color: d.payload.color || '#1e90ff' }}>OAS: +{d.value}bp</div>
                </div>
              );
            }} />
...
export function PnlAttribution() {
  const chartData = [
    ...PNL_DATA.map(d => ({ ...d, fill: d.pnl >= 0 ? '#1e90ff' : 'var(--bn-red)' })),
            ...
            <Tooltip content={(props: any) => {
              ...
              return (
                <div style={{ background: 'var(--bn-bg2)', border: BD, borderRadius: 3, padding: '6px 10px', fontFamily: 'JetBrains Mono,monospace', fontSize: 11 }}>
                  ...
                  <div style={{ color: isPos ? '#1e90ff' : 'var(--bn-red)', fontWeight: 600 }}>{isPos ? '+' : ''}${d.value}K</div>
                </div>
              );
            }} />
