...
/** True when `node_modules/@starui` holds architecture-bucket tarballs (not only nested workspace apps). */
function hasStaruiBucketPackages(nmPath) {
  const staruiDir = join(nmPath, '@starui');
...
/**
 * Closest ancestor whose `node_modules/@starui` has bucket tarballs.
 * Nested `apps/` workspaces install buckets on the app; `apps/node_modules/@starui`
...
/** `node_modules/@starui/<bucketShort>` for the nearest ancestor that installed that bucket. */
function findBucketNodeModules(appDir, bucketShort) {
  let dir = resolve(appDir);
  for (let i = 0; i < 8; i++) {
    const bucketDir = join(dir, 'node_modules', '@starui', bucketShort);
  ...
  return join(findStaruiPackageRoot(appDir), 'node_modules', '@starui', bucketShort);
}
...
function staruiTailwindContent(appDir) {
  ...
  for (const root of collectStaruiInstallRoots(appDir)) {
    const nm = join(root, 'node_modules', '@starui');
    installedGlobs.push(
