...
export default defineConfig({
  ...
  webServer: [
    {
      command: 'npm --prefix apps run dev -w @starui/demo-react',
      port: 5190,
      reuseExistingServer: true,
      timeout: 90_000,
    },
    {
      command: 'npm --prefix apps run dev -w @starui/demo-configservice-react',
      port: 5191,
      reuseExistingServer: true,
      timeout: 90_000,
    },
    {
      command: 'npm --prefix apps run dev -w @starui/markets-ui-react-reference',
      port: 5174,
      reuseExistingServer: true,
      timeout: 90_000,
    },
    {
      command: 'npm --prefix apps run dev -w @starui/e2e-browser-blotter',
      port: 5180,
      reuseExistingServer: true,
      timeout: 90_000,
    },
    {
      // Apps build from source by default — vite aliases @starui/grid straight
    ...
    },
    {
      ...
      command: 'npm run build --workspace=@starui/host-data && npm run build --workspace=@starui/widgets-react && npm --prefix apps run dev -w @starui/platform-hooks-demo -- --no-open --force',
      port: 5214,
    ...
    },
    {
      ...
      command: 'npm run build --workspace=@starui/widgets-react && npm --prefix apps run dev -w @starui/stomp-marketsgrid-minimal -- --no-open --force',
      port: 5213,
      reuseExistingServer: !process.env.CI,
      timeout: 120_000,
    },
