 ...
 *
 * This factory:
 *   1. Builds the worker URL pointing at `./worker/defaultEntry.ts`
 ...
 *
 * Worker URL constraint (Vite):
 *   Vite's worker plugin scans for the literal
 *     `new SharedWorker(new URL('./...', import.meta.url), {...})`
 *   at the call site. That static-analysis works regardless of
 *   which package the call site lives in — moving it inside
 *   `@stargrid/host-data` is fine; `import.meta.url` resolves
 *   relative to this file at build time and the worker chunk emits
 *   cleanly. Tested in dev (Vite ≥ 5) and prod builds.
 *
 * Escape hatch:
...
import { createConfigManager, type ConfigManager } from '@stargrid/host-config';
...
export function createDataServicesClient(
  ...
  const workerUrl = new URL('../worker/defaultEntry.ts', import.meta.url);
  ...
  worker.addEventListener('error', (ev) => {
    ...
    console.error('[@stargrid/host-data] SharedWorker error event', ev);
  });
