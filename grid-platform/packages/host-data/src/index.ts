/**
 * @stargrid/host-data — public entry.
 *
 * The runtime is the live surface. The root entry re-exports runtime
 * types so `@stargrid/host-data` continues to give consumers a
 * usable barrel. For specific entry points use the subpath exports:
 *
 *   `@stargrid/host-data/runtime`               — protocol types + main-thread helpers
 *   `@stargrid/host-data/runtime/client`        — the SharedWorkerDataServicesClient
 *   `@stargrid/host-data/runtime/sharedWorker`  — installSharedWorkerHub + SharedWorkerDataServicesHub
 *
