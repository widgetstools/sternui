/**
 * @stargrid/grid/customizer — React UI for the MarketsUI grid customizer.
 *
 * Hosts every React surface that previously lived under
 * `@stargrid/engine/src/{ui,hooks,modules}` (extracted in PR-8). Consumers
 * are `@starui/markets-grid` and any host app that needs the
 * settings-panel primitives, hooks, or module definitions.
 *
 * `@stargrid/engine` remains framework-agnostic vanilla TS: GridPlatform,
