...
import { createConfigServiceStorage } from '@stargrid/host-config';
...
export interface UseHostedIdentityArgs {
   ...
   * `@stargrid/openfin-platform/config` entry point. Pass an explicit
 ...
 * `LOGGED_IN_USER_ID` in `@stargrid/host`. The `defaultUserId`
 * arg below is retained on the public API for back-compat but is
 * ignored at runtime so persistence always lands under the same scope.
 */
 ...
 * widgets-react does not pull `@stargrid/openfin-platform` (and its
...
async function loadHostConfigManager(): Promise<ConfigManager | null> {
  try {
    ...
    const mod = (await import(
      /* @vite-ignore */ '@stargrid/openfin-platform/config' as string
