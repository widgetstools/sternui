...
import type { MarketsGridProps, MarketsGridHandle, StorageAdapterFactory } from '@stargrid/grid';
import type { AppDataLookup, StorageAdapter } from '@stargrid/engine';
import {
  ...
  useAppDataStore,
  useDataServices,
} from '@stargrid/host-data-react/runtime';
import { composeRowId, getValueByPath } from '@stargrid/types';
...
export function MarketsGridContainer<TData extends Record<string, unknown> = Record<string, unknown>>(
  ...
  // `composeRowId` in @stargrid/types). We pass the raw shape
  ...
  if (activeId && !activeRow.loading && rowIdField && columnDefs) {
    ...
    const userAdminActions = (marketsGridProps as { adminActions?: import('@stargrid/grid').AdminAction[] }).adminActions ?? [];
    const adminActionsWithRefresh: import('@stargrid/grid').AdminAction[] = [
      {
        id: 'refresh-provider',
        label: 'Refresh',
