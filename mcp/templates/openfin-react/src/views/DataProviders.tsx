 ...
 *
 * Popout target for `openProviderEditorPopout()`. The popout helper
 * passes `?id=<providerId>` when editing an existing row; we read it
 * via `URLSearchParams` and forward as `initialProviderId`.
 *
...
function DataProviders() {
  ...
  useEffect(() => {
    ...
    document.title = "Data Providers · {{name}}";
    return () => { document.title = prev; };
  }, []);

  // index.css sets `body { padding: 10px }` for the home page chrome.
  // That leaks here as 10px gutters around the popout and pushes
  // h-screen content past the viewport — producing OS-default
  // page-level scrollbars. Zero body padding/margin on mount so the
  // editor fills the popout flush; restore on unmount.
  useEffect(() => {
