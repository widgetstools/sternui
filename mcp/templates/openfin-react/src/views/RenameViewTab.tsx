 ...
 *
 * Runtime rename: workspace tabstrip reads `document.title` by default.
 * `View.updateOptions({title})` is a no-op at runtime. We set
 * `document.title` on the target view via `executeJavaScript`, and
 * persist into `customData.savedTitle` so the rename survives a
 * workspace save/restore round-trip.
 */
