 ...
 *   1. Skip if its `name` doesn't start with `@starui/`.
...
export interface BuildAliasesOptions {
  ...
  packagesRoot: string;
  /** Package-name prefix to scan. Default: `@starui/`. */
  scope?: string;
  /**
   * Package names to skip (e.g. ones that don't survive source-
   * aliasing because they ship pre-bundled worker chunks).
   * Provide either bare names ("@starui/foo") or subpaths
   * ("@starui/foo/bar"). Caller layers explicit overrides for
   * these.
   */
...
/**
 * Build Vite alias entries by walking every `@starui/*` package.
 *
...
export function buildPackageAliases(opts: BuildAliasesOptions): AliasEntry[] {
  ...
  for (const dirName of pkgDirs) {
    ...
    // so a plain `@starui/PKG` import still resolves.
