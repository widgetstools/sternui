...
| **STOMP broker** | [`stomp-view-server`](../../apps/tarball/stomp-view-server) on `ws://localhost:8081` — matches `src/stompProvider.ts` |
...
Use this app to verify **hub bootstrap**, **catalog persistence**, and **cfg-free STOMP attach**. For authoring providers in UI, see `apps/workspace/dataprovider-editor` or `apps/workspace/markets-grid-lab`. For **AppData bootstrap + grid event callbacks** with mock data (no broker), see [`apps/workspace/platform-hooks-demo`](../platform-hooks-demo/README.md).
