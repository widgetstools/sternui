...
export const FEATURE_GUIDES: Record<string, FeatureGuide> = {
  ...
  stress: {
    ...
    summary: '50k rows × 400 columns with grouping, conditional styling, and high-tick live updates — CSRM vs SSRM.',
    whatWhy:
      'Stress Test mounts a **50,000 × 400** blotter with row grouping (Class → Sector), aggregations, conditional styling, Excel/preset formatters, calculated columns, column groups, and quick-filter pills. **Live ticks run at ~200 ms** (Phase 4c high-tick) so you can compare CSRM vs SSRM under continuous deltas. Flip **Use SSRM** in the header to remount on the same shape — MarketsGrid stress variants use **CustomSSRMGrid** (RowMirror). Synthetic `S000…` columns are valueGetter-backed (wide scroll / paint stress without bloating row objects). Pause Live ticks in the Demo Console if you only want first-paint / layout stress.',
    trySteps: [
