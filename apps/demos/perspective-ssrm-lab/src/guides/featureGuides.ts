...
export const FEATURE_GUIDES: Record<string, FeatureGuide> = {
  ...
  'ssrm-engine': {
    ...
    trySteps: [
      {
        text: 'Press "Sort by Carry" — a calculated column that is NULL wherever its guard fails — then scroll to the bottom, and flip the sort.',
        hint: 'The nulls stay LAST IN BOTH DIRECTIONS. AG\'s own comparator returns -1 for a null and the grid multiplies by the direction, so on CSRM nulls sort FIRST ascending; this engine does not, because a NaN price above the best bid is worse. `calc_pnlPct` guards on `marketValue > 0`, true for every row of this book — which is why the button points at Carry and not at it.',
      },
      {
