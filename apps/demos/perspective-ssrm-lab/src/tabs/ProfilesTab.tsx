...
function PresetGridView({
  ...
  const onProfilesReady =
    preset.demoProfiles && preset.demoProfiles.length > 0 && preset.activeDemoProfileId
      ? installDemoProfiles
      : undefined;
  const { table, status, reason, onReady } = useLabPerspectiveRows(
    'profiles',
  ...
  return (
    ...
    >
      <div className="flex min-h-0 flex-1 flex-col">
        {/* One Table per preset, so switching presets is a fresh provider and
            a fresh book — the same isolation the CSRM lab got from its
            per-preset providerId. No remount `key`: the engine never changes
            here, and exactly one grid may mount per GridPlatform. */}
        {table ? (
          <MarketsGrid
            gridId={preset.id}
