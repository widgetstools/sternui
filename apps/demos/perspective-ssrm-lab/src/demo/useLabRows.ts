...
/**
 * CLIENT-SIDE row supply — kept ONLY for the Stress tab's baseline surfaces.
 *
 * Every feature tab in this app is on the pull path and uses
 * `useLabPerspectiveRows` instead. This survives because the Stress tab
 * benchmarks MarketsGrid AGAINST plain AG Grid and the FINOS Perspective
 * viewer, and those two are client-side by construction — that is the entire
 * point of them as controls. Feeding them from a Table would measure the
 * Table, not them.
 *
 * Do not reach for this from a feature tab. If a tab needs rows in the window,
 * that is a signal it is not on the pull path.
 *
